...
program
  ...
  .option(
    '-p, --products [PRODUCT_IDS...]',
    'List of space-separated product IDs. Example: admin github developers'
  )
  .option(
    '-c, --currentServerReleases',
    'Also add AE versioning to conditionals for current Enterprise Server releases.'
  )
  .option(
    '-t, --translations',
    'Run the script on content and data in translations, too.'
  )
  .parse(process.argv)
...
if (program.currentServerReleases) {
  console.log(
    `✅ Adding AE versioning based on GHES ${program.ghesRelease} versioning and all currently supported GHES versions`
  )
} else {
  console.log(
    `✅ Adding AE versioning based only on GHES ${program.ghesRelease} versioning`
  )
}
...
const getEnterpriseServerConditional = new RegExp(
  `currentVersion (\\S+?) "${getEnterpriseServerNumber.source}"`
)
...
function walkData(dirPath, contentFiles) {
  return walk(dirPath, { includeBasePath: true, directories: false })
    .filter(
      (file) =>
        file.includes('/data/reusables') || file.includes('/data/variables')
    )
    .filter((file) => !file.endsWith('README.md'))
}
...
function updateLiquid(content, file) {
  ...
  allConditionals.forEach((conditional) => {
    ...
    const enterpriseServerMatch = newConditional.match(
      getEnterpriseServerConditional
    )

    // Add AE conditional to any `currentVersion != "free-pro-team@latest"`
    if (
      program.currentServerReleases &&
      newConditional.match(notDotcomConditional)
    ) {
      if (
        enterpriseServerMatch &&
        !doesReleaseSatisfyConditional(enterpriseServerMatch)
      )
        return

      newConditional = newConditional.replace(
        /( ?)%}/,
        `$1or ${githubAEConditional} %}`
      )
    ...
    if (
      program.currentServerReleases &&
      newConditional.match(allGHESVersionsConditional)
    ) {
      if (
        enterpriseServerMatch &&
        !doesReleaseSatisfyConditional(enterpriseServerMatch)
    ...
    const releaseSatisfiesConditional = doesReleaseSatisfyConditional(
      enterpriseServerMatch
    )
    ...
    newConditional = newConditional.replace(
      enterpriseServerMatch[0],
      `${enterpriseServerMatch[0]} or ${githubAEConditional}`
    )
