...
const externalScalars = require('../../../lib/graphql/non-schema-scalars').map(
  (scalar) => {
  ...
  await Promise.all(
    schemaAST.definitions.map(async (def) => {
      ...
      if (def.name.value === 'Query') {
        await Promise.all(
          def.fields.map(async (field) => {
            ...
            query.description = await helpers.getDescription(
            ...
            await Promise.all(
              field.arguments.map(async (arg) => {
                ...
                queryArg.description = await helpers.getDescription(
      ...
      if (def.name.value === 'Mutation') {
        await Promise.all(
          def.fields.map(async (field) => {
            ...
            mutation.description = await helpers.getDescription(
              field.description.value
            )
            ...
            await Promise.all(
              field.arguments.map(async (field) => {
                ...
                inputField.href = helpers.getFullLink(
                  inputField.kind,
                  inputField.id
                )
            ...
            await Promise.all(
              mutationReturnFields.fields.map(async (field) => {
                ...
                returnField.href = helpers.getFullLink(
                  returnField.kind,
                  returnField.id
                )
      ...
      if (def.kind === 'ObjectTypeDefinition') {
        ...
        if (def.interfaces.length) {
          await Promise.all(
            def.interfaces.map(async (graphqlInterface) => {
              ...
              objectInterface.href = helpers.getFullLink(
                'interfaces',
                objectInterface.id
              )
        ...
        if (def.fields.length) {
          await Promise.all(
            def.fields.map(async (field) => {
              ...
              objectField.description = await helpers.getDescription(
                field.description.value
              )
              ...
              objectField.href = helpers.getFullLink(
                objectField.kind,
                objectField.id
              )
        ...
        if (objectImplements.length)
          object.implements = sortBy(objectImplements, 'name')
      ...
      if (def.kind === 'InterfaceTypeDefinition') {
        ...
        graphqlInterface.href = helpers.getFullLink(
          'interfaces',
          graphqlInterface.id
        )
        graphqlInterface.description = await helpers.getDescription(
        ...
        if (def.fields.length) {
          await Promise.all(
            def.fields.map(async (field) => {
              ...
              interfaceField.description = await helpers.getDescription(
                field.description.value
              )
              interfaceField.type = helpers.getType(field)
              interfaceField.id = helpers.getId(interfaceField.type)
              interfaceField.kind = helpers.getTypeKind(
                interfaceField.type,
                schema
              )
              interfaceField.href = helpers.getFullLink(
                interfaceField.kind,
                interfaceField.id
              )
      ...
      if (def.kind === 'EnumTypeDefinition') {
        ...
        graphqlEnum.description = await helpers.getDescription(
          def.description.value
        )
        ...
        await Promise.all(
          def.values.map(async (value) => {
            ...
            enumValue.description = await helpers.getDescription(
      ...
      if (def.kind === 'InputObjectTypeDefinition') {
        ...
        inputObject.description = await helpers.getDescription(
          def.description.value
        )
        ...
        if (def.fields.length) {
          await Promise.all(
            def.fields.map(async (field) => {
              ...
              inputField.name = field.name.value
              inputField.description = await helpers.getDescription(
                field.description.value
              )
              inputField.type = helpers.getType(field)
              inputField.id = helpers.getId(inputField.type)
      ...
      if (def.kind === 'ScalarTypeDefinition') {
        ...
        scalar.deprecationReason = await helpers.getDeprecationReason(
          def.directives,
          scalar
        )
        scalar.preview = await helpers.getPreview(
          def.directives,
          scalar,
          previewsPerVersion
        )
