...
async function getDeprecationReason(directives, schemaMember) {
  ...
  const deprecationDirective = directives.filter(
    (dir) => dir.name.value === 'deprecated'
  )
...
async function getPreview(directives, schemaMember, previewsPerVersion) {
  ...
  // it's possible for a schema member to be deprecated and under preview
  const previewDirective = directives.filter(
    (dir) => dir.name.value === 'preview'
  )
  ...
  const preview = previewsPerVersion.find((p) =>
    p.toggled_by.includes(previewName)
  )
  if (!preview)
    console.error(`cannot find "${previewName}" in graphql_previews.yml`)
