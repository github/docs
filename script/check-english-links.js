...
program
  ...
  .option(
    '-p, --path <PATH>',
    'Provide an optional path to check. Best used with --dry-run. If not provided, defaults to the homepage.'
  )
  .parse(process.argv)
...
const enterpriseReleasesToSkip = new RegExp(
  `${root}.+?[/@](${deprecated.join('|')})/`
)
...
async function main() {
  ...
  const brokenLinks = result.filter((link) => link.state === 'BROKEN')

  // Links to retry individually.
  const linksToRetry = brokenLinks.filter(
    (link) => !link.status || retryStatusCodes.includes(link.status)
  )
...
function displayBrokenLinks(brokenLinks) {
  ...
  allStatusCodes.forEach((statusCode) => {
    const brokenLinksForStatus = brokenLinks.filter(
      (x) => x.status === statusCode
    )

    console.log(
      `## Status ${statusCode}: Found ${brokenLinksForStatus.length} broken links`
    )
    console.log('```')
