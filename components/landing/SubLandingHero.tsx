      ...
      <ul className="list-style-none d-flex flex-nowrap overflow-x-scroll px-2 feature-track">
        <li className="px-2 d-flex flex-shrink-0">
          <div className="d-inline-block Box p-5 bg-gradient--blue-pink color-text-inverse">
            <div className="circle color-text-inverse d-inline-flex" style={{ border: "2px white solid" }}>
              <StarFillIcon className="v-align-middle m-2" size={24} />
            </div>
            <h3 className="font-mktg h3-mktg my-4">{featuredTrack.title}</h3>
            <div className="lead-mktg color-text-inverse f5 my-4">{featuredTrack.description}</div>          
            {featuredTrack && <a className="d-inline-block border color-border-inverse color-text-inverse px-4 py-2 f5 no-underline text-bold" role="button" href={`${featuredTrack.guides[0].href}?learn=${featuredTrack.trackName}`}>
              <span className="mr-2">
          ...
          <li className="px-2 d-flex flex-shrink-0">
            <a href="{{ guide.href }}?learn={{ featuredTrack.trackName }}" className="d-inline-block Box p-5 color-bg-primary color-border-primary no-underline">
              <div className="d-flex flex-justify-between flex-items-center">
                <div className="circle color-bg-primary color-text-link border-gradient--pink-blue-dark d-inline-flex">
                  <span className="m-2 f2 lh-condensed-ultra text-center text-bold step-circle-text"></span>
                </div>
                <div className="color-text-tertiary h6 text-uppercase"></div>
              </div>
              <h3 className="font-mktg h3-mktg my-4 color-text-primary"></h3>
              <div className="lead-mktg color-text-secondary f5 my-4 truncate-overflow-8"></div>
            </a>
          </li>
      </ul>
