...
1. Paste the text below, replacing the email used in the example with your {% data variables.product.product_name %} email address.
   ...
   **Note:** If you are using a legacy system that doesn't support the Ed25519 algorithm, use:
   ...
   ```shell
   > Enter a file in which to save the key ($USER/.ssh/id_ALGORITHM): [Press enter]
   ```
   ...
   ```powershell
   > Enter a file in which to save the key (C:\Users\%Username%\.ssh\id_ALGORITHM):[Press enter]
   ```
...
1. When you are prompted to "Enter a file in which to save the key," press Enter to accept the default file location.
   ...
   ```shell
   > Enter a file in which to save the key (C:\Users\%Username%\.ssh\id_{% ifversion ghae %}ecdsa{% else %}ed25519{% endif %}_sk): [Press enter]
   ```
   ...
   ```shell
   > Enter a file in which to save the key ($HOME/.ssh/id_{% ifversion ghae %}ecdsa{% else %}ed25519{% endif %}_sk):[Press enter]
   ```
