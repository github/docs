...
1. Optionally, enable required pull request reviews.
   - Under "Protect matching branches", select **Require pull request before merging**.
     ![Pull request review restriction checkbox](/assets/images/help/repository/PR-reviews-required.png)
   - Click the **Required approving reviews** drop-down menu, then select the number of approving reviews you'd like to require on the branch. (Note: It is not possible to uncheck required approving reviews, or to select zero approvers. If you are trying to to restrict push and force push to branches and require branching and pull requests, you should try ________ instead.)
