...
intro: 'Understanding the differences between {{ site.data.variables.product.prodname_github_app }}s and {{ site.data.variables.product.prodname_oauth_app }}s will help you decide which app you want to create. An {{ site.data.variables.product.prodname_oauth_app }} acts as a GitHub user, whereas a {{ site.data.variables.product.prodname_github_app }} uses its own identity when installed on an organization or on repositories within an organization.'
redirect_from:
...
Account owners can use a {{ site.data.variables.product.prodname_github_app }} in one account without granting access to another. For example, you can install a third-party build service on your employer's organization, but decide not to grant that build service access to repositories in your personal account. A GitHub App remains installed if the person who set it up leaves the organization.
