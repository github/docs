...
To help your project contributors do good work, you can add a file with contribution guidelines to your project repository's root, `docs`, or `.github` folder. When someone opens a pull request or creates an issue, they will see a link to that file. Also link to the contributing guidelines is shown on the repository's contribute page.
...
3. Decide whether to store your contributing guidelines in your repository's root, `docs`, or `.github` directory. Then, in the filename field, type the name and extension for the file. Contributing guidelines filenames are not case sensitive, and files are rendered using rich text format if it's extension is in supported format. For more information, see "[Rendering differences in prose documents](/github/managing-files-in-a-repository/rendering-differences-in-prose-documents)."
  ![New file name](/assets/images/help/repository/new-file-name.png)
    ...
    - If a repository contains more than one *CONTRIBUTING* file, then the file shown in links is chosen in order from `.github` directory, the repository's root directory and then the `docs` directory.
