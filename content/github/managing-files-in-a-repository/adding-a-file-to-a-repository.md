...
intro: 'You can upload and commit an existing file to a {{ site.data.variables.product.product_name }} repository. Drag and drop a file to any directory in the file tree, or upload files from the repository''s main page.'
redirect_from:
...
Files that you add to a repository via a browser are limited to {{ site.data.variables.large_files.max_github_browser_size }} per file. You can add larger files, up to {{ site.data.variables.large_files.max_github_size }} each, via the command line. For more information, see "[Adding a file to a repository using the command line](/articles/adding-a-file-to-a-repository-using-the-command-line)."

{% tip %}

**Tips:**
- You can upload multiple files to {{ site.data.variables.product.product_name }} at the same time.
