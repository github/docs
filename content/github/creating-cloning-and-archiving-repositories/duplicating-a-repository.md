...
Before you can duplicate a repository and push to your new copy, or _mirror_, of the repository, you must [create the new repository](/articles/creating-a-new-repository) on {{ site.data.variables.product.product_location }}. In these examples, `exampleuser/new-repository` or `exampleuser/mirrored` are the mirrors.
...
### Mirroring a repository that contains {{ site.data.variables.large_files.product_name_long }} objects
...
3. Navigate to the repository you just cloned.
  ...
  $ cd <em>old-repository</em>.git
  ```
4. Pull in the repository's {{ site.data.variables.large_files.product_name_long }} objects.
  ```shell
  $ git lfs fetch --all
  ```
5. Mirror-push to the new repository.
  ...
  $ git push --mirror https://{{ site.data.variables.command_line.codeblock }}/<em>exampleuser</em>/<em>new-repository</em>.git
  ```
6. Push the repository's {{ site.data.variables.large_files.product_name_long }} objects to your mirror.
  ```shell
