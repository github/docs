...
intro: >-
  Every action the Copilot agent takes—thinking, writing code, running tools—is
  emitted as a **session event** you can subscribe to. This guide is a
  field-level reference for each event type so you know exactly what data to
  expect without reading the SDK source.
versions:
  fpt: '*'
  ghec: '*'
redirect_from:
