...
1. For jobs that continue to run, the server re-evaluates `if` conditions for the unfinished steps. If the condition evaluates to `true`, the step continues to run. You can use [cancelled()](/en/learn-github-actions/expressions#cancelled) especially for the cancel condition.
