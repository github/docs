...
intro: 'You can view, search, and download the logs for each job in a workflow run. {{ site.data.variables.product.product_name }} stores full build logs and artifacts for 90 days.'
...
In addition to the steps configured in the workflow file, {{ site.data.variables.product.prodname_dotcom }} adds two additional steps to each job to set up and complete the job's execution. These steps are logged in the workflow run with the names "Set up job" and "Complete job".
