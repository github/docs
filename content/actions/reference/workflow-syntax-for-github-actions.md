...
The `branches`, `branches-ignore`, `tags`, and `tags-ignore` keywords accept glob patterns that use characters like `*`, `**`, `+`, `?`, `!` and others to match more than one branch or tag name. If your branch name contains one of these characters, you need to *escape* the character with `\` for a literal match. For more information, see the "[Filter pattern cheat sheet](#filter-pattern-cheat-sheet)."
