  ...
  weekday_job:
    ...
    steps:
      - name: "Hello world when it's Monday"
        ...
        run: echo "Hello world $FIRST_NAME $middle_name $Last_Name, sounds like you have a case of the Mondays!"
        env:
          FIRST_NAME: Mona
          middle_name: The
          Last_Name: Octocat
...
If you use the runner operating system's method for reading environment variables in a workflow file's `run` key (as shown in the example above), the variable is substituted in the runner operating system after the job is sent to the runner. For other places in a workflow file you must use the `env` context, because workflow keys like `if` require the variable to be substituted during workflow processing before it is sent to the runner.
