...
We have do support a flow that allows cross-repository access within {% data variables.product.product_name %} for using [GitHub Actions stored in internal repositories](/actions/creating-actions/sharing-actions-and-workflows-with-your-enterprise). Currently, the only way to perform other privileged cross-repository interactions is to place a {% data variables.product.prodname_dotcom %} authentication token or SSH key as a secret within the workflow. Because many authentication token types do not allow for granular access to specific resources, there is significant risk in using the wrong token type, as it can grant much broader access than intended.
