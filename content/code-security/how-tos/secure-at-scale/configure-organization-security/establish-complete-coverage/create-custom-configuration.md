...
1. In the "{% data variables.product.prodname_secret_scanning_caps %}" section of the security settings table, choose whether you want to enable, disable, or keep the existing settings for the following security features:
    * Alerts. To learn about {% data variables.secret-scanning.alerts %}, see [AUTOTITLE](/code-security/secret-scanning/introduction/about-secret-scanning).
    ...
    * Push protection. To learn about push protection, see [AUTOTITLE](/code-security/secret-scanning/introduction/about-push-protection).
1. Optionally, under "Push protection", choose whether you want to grant bypass privileges{% ifversion push-protection-org-enterprise-exemptions %} or exemptions{% endif %} for selected actors in your organization. {% ifversion push-protection-org-enterprise-exemptions %}Bypass privileges allow actors to bypass push protection blocks, as well as review and approve bypass requests from other contributors, while exemptions stop push protection from running on pushes by selected actors.{% else %}By assigning bypass privileges, selected organization members can bypass push protection, and there is a review and approval process for all other contributors.{% endif %} For further guidance on how to configure this setting, see [AUTOTITLE](/code-security/secret-scanning/using-advanced-secret-scanning-and-push-protection-features/delegated-bypass-for-push-protection/enabling-delegated-bypass-for-push-protection#enabling-delegated-bypass-for-an-organization).
