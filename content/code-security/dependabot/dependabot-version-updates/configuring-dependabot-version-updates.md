...
By default only direct dependencies that are explicitly defined in a manifest are kept up to date by {% data variables.product.prodname_dependabot_version_updates %}. If you wish to receive updates for indirect dependencies defined in lock files as well, you can enable this with the [`allow` configuration option](/code-security/dependabot/dependabot-version-updates/configuration-options-for-the-dependabot.yml-file#allow).
