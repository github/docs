...
It is possible for the same alert to be generated by different analysis origins, leading to multiple instances of the same code scanning alert in a repository. If an alert exists for multiple analysis origins, the alert details page will display the analysis origin name at the top of the page, allowing you to differentiate between multiple instances of the same alert. In the "Affected branches" section, a tag icon will also appear next to any branch where the alert has multiple analysis origins. You can hover over the icon to see the names of all the analysis origins for which the alert exists.  If an alert only has one analysis origin, the analysis origin will not display on the alert details page at all. 
