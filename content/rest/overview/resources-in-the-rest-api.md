...
By default, all requests to `{{ site.data.variables.product.api_url_code }}` receive the **v3** [version](/v3/versions) of the REST API.
...
There are two ways to authenticate through {{ site.data.variables.product.product_name }} API v3.  Requests that
...
see [events](/v3/activity/events/) for example.
