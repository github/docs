...
import type { AutocompleteSearchResponse, SearchValidationErrorEntry } from '@/search/types'
...
describeIfElasticsearchURL('search/autocomplete v1 middleware', () => {
  ...
  test('basic search', async () => {
    ...
    sp.set('query', 'fo')
    const res = await get('/api/search/autocomplete/v1?' + sp.toString())
    expect(res.statusCode).toBe(200)
    const results: AutocompleteSearchResponse = JSON.parse(res.body)
  ...
  test('invalid version', async () => {
    ...
    const errorResponse: SearchValidationErrorEntry = JSON.parse(res.body).error
  ...
  test('variations on version name', async () => {
    ...
    // Test with 'enterprise-cloud' version
    ...
    // Test with 'free-pro-team@latest' version
