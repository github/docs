...
const metaData = JSON.parse(await readFile('src/rest/data/meta.json', 'utf8'))
...
export async function getSchemas(directory = OPEN_API_RELEASES_DIR) {
  ...
  for (const file of openAPIConfigs) {
    ...
    const releaseMatch = Object.keys(metaData.versionMapping).find((name) =>
      fileBaseName.startsWith(name)
    )
    if (!releaseMatch) {
      throw new Error(
        `🛑 The file ${fileBaseName} does not match any known docs version name. (not one of ${Object.keys(
          metaData.versionMapping
        )})`
      )
    }
    const docsName =
      metaData.versionMapping[fileBaseName] ||
      fileBaseName.replace(releaseMatch, metaData.versionMapping[releaseMatch])
