...
| `invocation.workingDirectory.uri` | **Opcional.** Esse campo é usado somente quando `checkout_uri` (somente API de upload de SARIF) ou `checkout_path` (somente % data variables.product.prodname_actions %}) não são fornecidos. O valor é usado para converter URIs absolutos usados em [objetos `physicalLocation`](#physicallocation-object) em URIs relativos. Para obter mais informações, confira "[Como especificar a raiz para arquivos de origem](#specifying-the-root-for-source-files)".|
