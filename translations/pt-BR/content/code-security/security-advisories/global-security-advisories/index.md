---
title: Trabalhar com avisos de segurança global do Banco de Dados de Avisos do GitHub
shortTitle: Global security advisories
intro: 'Navegue pelo {% data variables.product.prodname_advisory_database %} e envie melhorias em qualquer aviso de segurança global.'
versions:
  fpt: '*'
  ghes: '*'
  ghae: '*'
  ghec: '*'
topics:
  - Security advisories
  - Vulnerabilities
  - Repositories
  - CVEs
children:
  - /about-the-github-advisory-database
  - /about-global-security-advisories
  - /browsing-security-advisories-in-the-github-advisory-database
  - /editing-security-advisories-in-the-github-advisory-database
ms.openlocfilehash: ff0ba15abd3a404cbdb8ce97347d8dd5a4090280
ms.sourcegitcommit: 27882d9b3f19979c817c25952a2fb4dc4c6f0a65
ms.translationtype: HT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/27/2022
ms.locfileid: '148113951'
---

