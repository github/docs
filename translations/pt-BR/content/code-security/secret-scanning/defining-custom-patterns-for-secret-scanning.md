...
É possível definir padrões personalizados para sua empresa, organização ou repositório. O {% data variables.product.prodname_secret_scanning_caps %} dá suporte a até {%- ifversion fpt or ghec or ghes > 3.3 or ghae-issue-7297 %} 500 padrões personalizados para cada organização ou conta empresarial e até 100 padrões personalizados por repositório.
{%- elsif ghes = 3.2 %} 20 padrões personalizados para cada organização ou conta empresarial e por repositório.
{%- else %} 100 padrões personalizados para cada organização ou conta empresarial e 20 por repositório.
{%- endif %}

{% ifversion ghes < 3.3 %} {% note %}

**Observação:** durante a versão beta, existem algumas limitações de uso dos padrões personalizados na {% data variables.product.prodname_secret_scanning %}:
...
* Não há API para criar, editar ou excluir padrões personalizados. No entanto, os resultados dos padrões personalizados são retornados na [API de alertas de verificação de segredos](/rest/reference/secret-scanning).
...
1. Em "Padrões personalizados da verificação de segredos", clique em {% ifversion ghes = 3.2 %}**Novo padrão personalizado**{% else %}**Novo padrão**{% endif %}.
...
1. Clique em **Sim, excluir este padrão**.
   ...
   ![Como confirmar a exclusão de um padrão personalizado da {% data variables.product.prodname_secret_scanning %} ](/assets/images/help/repository/secret-scanning-confirm-deletion-custom-pattern.png) {%- elsif ghes = 3.2 %}
1. À direita do padrão personalizado que deseja remover, clique em **Remover**.
