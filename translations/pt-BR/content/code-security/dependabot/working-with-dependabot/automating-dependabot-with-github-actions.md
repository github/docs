...
{% ifversion fpt or ghec or ghes > 3.3 or ghae-issue-5792 %} Para os fluxos de trabalho iniciados pelo {% data variables.product.prodname_dependabot %} (`github.actor == 'dependabot[bot]'`) por meio dos eventos `pull_request`, `pull_request_review`, `pull_request_review_comment`, `push`, `create`, `deployment` e `deployment_status`, as seguintes restrições se aplicam: {% endif %}
...
{% ifversion fpt or ghec or ghes > 3.3 or ghae-issue-5792 %} Para os fluxos de trabalho iniciados pelo {% data variables.product.prodname_dependabot %} (`github.actor == 'dependabot[bot]'`) por meio do evento `pull_request_target`, se a referência base da solicitação de pull tiver sido criada pelo {% data variables.product.prodname_dependabot %} (`github.actor == 'dependabot[bot]'`), o `GITHUB_TOKEN` será somente leitura e os segredos não ficarão disponíveis.
{% endif %}
...
**Observação:** se o administrador do site tiver substituído as restrições para o {% data variables.product.prodname_dependabot %} no {% data variables.product.product_location %}, você poderá usar `pull_request` em vez de `pull_request_target` nos fluxos de trabalho a seguir.
...
Se você deseja permitir que os mantenedores marquem determinadas solicitações de pull para mesclagem automática,você pode usar a funcionalidade de mesclagem automática de {% data variables.product.prodname_dotcom %} Isto permite que o pull request seja mesclado quando todos os testes e aprovações forem cumpridos com sucesso. Para obter mais informações sobre a mesclagem automática, confira "[Mesclar uma solicitação de pull automaticamente](/pull-requests/collaborating-with-pull-requests/incorporating-changes-from-a-pull-request/automatically-merging-a-pull-request)".
