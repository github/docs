...
* Você deve criar um token de acesso pessoal (PAT) na sua empresa que pode criar e gravar em repositórios nas organizações de destino. Para obter mais informações, confira "[Como criar um token de acesso pessoal](/github/authenticating-to-github/creating-a-personal-access-token)".{% ifversion ghes %}
* Se você quiser sincronizar as ações empacotadas na organização `actions` no {% data variables.product.product_location %}, deverá ser um proprietário da organização `actions`.
...
1. Run the `actions-sync sync` command:
   ...
   * `--destination-token`: um token de acesso pessoal para a instância corporativa de destino.
