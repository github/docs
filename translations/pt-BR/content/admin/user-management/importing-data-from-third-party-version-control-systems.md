...
intro: 'Usando o conjunto de ferramentas git-import, é possível fazer importações dos controles de versão do Subversion, do Mercurial e do Team Foundation para repositórios do Git no {{ site.data.variables.product.prodname_ghe_server }}.'
redirect_from:
...
4. Rescreva os autores e branches usando o arquivo CSV:
  ...
  ```
5. Caso ainda não tenha feito isso, [crie um repositório vazio no {{ site.data.variables.product.prodname_ghe_server }}](/enterprise/{{ currentVersion }}/user/articles/creating-a-new-repository).
{{ site.data.reusables.command_line.switching_directories_procedural }}
7. Faça push do repositório importado para o {{ site.data.variables.product.prodname_ghe_server }}:
  ```shell
