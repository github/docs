...
intro: 'Como parte de um plano de recuperação de desastre, é possível proteger os dados de produção na {{ site.data.variables.product.product_location_enterprise }} configurando backups automatizados.'
versions:
  enterprise-server: '*'
---

### Sobre o {{ site.data.variables.product.prodname_enterprise_backup_utilities }}

O {{ site.data.variables.product.prodname_enterprise_backup_utilities }} é um sistema de backup a ser instalado em um host separado, que tira instantâneos de backup da {{ site.data.variables.product.product_location_enterprise }} em intervalos regulares em uma conexão de rede SSH segura. É possível usar um instantâneo para voltar uma instância do {{ site.data.variables.product.prodname_ghe_server }} a um estado anterior do host de backup.
...
Para obter informações mais detalhadas sobre recursos, requisitos e uso avançado, consulte o [LEIAME do {{ site.data.variables.product.prodname_enterprise_backup_utilities }}](https://github.com/github/backup-utils#readme).

### Pré-requisitos

Para usar o {{ site.data.variables.product.prodname_enterprise_backup_utilities }}, você deve ter um sistema host Linux ou Unix separado da {{ site.data.variables.product.product_location_enterprise }}.

Também é possível integrar o {{ site.data.variables.product.prodname_enterprise_backup_utilities }} a um ambiente para fins de armazenamento permanente em longo prazo de dados essenciais.

É recomendável que o host de backup e a {{ site.data.variables.product.product_location_enterprise }} estejam geograficamente distantes. Essa medida garante que os backups estejam disponíveis para recuperação em casos de grandes desastres ou falhas de rede no site primário.
...
### Instalar o {{ site.data.variables.product.prodname_enterprise_backup_utilities }}
...
1. Baixe a [versão mais recente do {{ site.data.variables.product.prodname_enterprise_backup_utilities }}](https://github.com/github/backup-utils/releases) e extraia o arquivo com o comando `tar`.
  ```shell
...
3. Defina o valor `GHE_HOSTNAME` para o nome de host ou endereço IP da instância primária do {{ site.data.variables.product.prodname_ghe_server }}.
...
5. Verifique a conectividade SSH com a {{ site.data.variables.product.product_location_enterprise }} usando o comando `ghe-host-check`.
  ```shell
  $ bin/ghe-host-check        
  ```
...
Para obter mais informações sobre uso avançado, consulte o [arquivo LEIAME do {{ site.data.variables.product.prodname_enterprise_backup_utilities }}](https://github.com/github/backup-utils#readme).
...
Se houver sobreposição de tentativas de backup, o comando `ghe-backup` será interrompido com uma mensagem de erro, informando a existência de um backup simultâneo. Nesse caso, é recomendável diminuir a frequência dos backups programados. Para obter mais informações, consulte a seção "Agendar backups" do [ arquivo LEIAME do {{ site.data.variables.product.prodname_enterprise_backup_utilities }}](https://github.com/github/backup-utils#scheduling-backups).

### Restaurar um backup

Em caso de interrupção prolongada ou evento catastrófico no site primário, é possível restaurar a {{ site.data.variables.product.product_location_enterprise }} provisionando outro appliance do {{ site.data.variables.product.prodname_enterprise }} e executando uma restauração no host de backup. Antes de restaurar um appliance, você deve adicionar a chave SSH do host de backup ao appliance de destino do {{ site.data.variables.product.prodname_enterprise }} como chave SSH autorizada.

Para restaurar a {{ site.data.variables.product.product_location_enterprise }} do instantâneo mais recente bem-sucedido, use o comando `ghe-restore`. Você verá um conteúdo semelhante a este:
...
**Observação:** as configurações de rede são excluídas do instantâneo de backup. Você deve configurar manualmente a rede no appliance de destino do {{ site.data.variables.product.prodname_ghe_server }} conforme o seu ambiente.

{% endnote %}

Você pode usar estas opções adicionais com o comando `ghe-restore`:
- O sinalizador `-c` substitui as configurações, os certificados e os dados de licença no host de destino, mesmo que já configurado. Omita esse sinalizador se você estiver configurando uma instância de preparo para fins de teste e se quiser manter a configuração no destino. Para obter mais informações, consulte a seção "Usar comandos de backup e restauração" do [LEIAME do {{ site.data.variables.product.prodname_enterprise_backup_utilities }}](https://github.com/github/backup-utils#using-the-backup-and-restore-commands).
