...
intro: 'Você pode hospedar seus próprios executores e personalizar o ambiente usado para executar trabalhos nos seus fluxos de trabalho do {{ site.data.variables.product.prodname_actions }}.'
redirect_from:
...
{{ site.data.reusables.github-actions.self-hosted-runner-description }} Os executores auto-hospedados podem ser físicos, virtuais, estar em um container, no local ou em uma nuvem.
...
### Você pode executar fluxos de trabalho nos executores hospedados em {{ site.data.variables.product.prodname_dotcom }} ou em executores auto-hospedados

Os executores auto-hospedados em {{ site.data.variables.product.prodname_dotcom }} oferecem uma maneira mais rápida e simples de executar seus fluxos de trabalho, enquanto os executores auto-hospedados são uma maneira altamente configurável de executar fluxos de trabalho em seu próprio ambiente personalizado.

**Executores hospedados no {{ site.data.variables.product.prodname_dotcom }}:**
...
- São gerenciados e mantidos por {{ site.data.variables.product.prodname_dotcom }}.
- Fornece uma instância limpa para cada execução de trabalho.
- Use minutos grátis no seu plano {{ site.data.variables.product.prodname_dotcom }}, com taxas por minuto aplicadas após exceder os minutos grátis.
...
- São grátis para usar com {{ site.data.variables.product.prodname_actions }}, mas você é responsável pelo custo de manutenção das suas máquinas executoras.
...
* A máquina pode comunicar-se com {{ site.data.variables.product.prodname_actions }}. Para obter mais informações, consulte "[Comunicação entre os executores auto-hospedados e {{ site.data.variables.product.prodname_dotcom }}](#communication-between-self-hosted-runners-and-github)".
...
### Comunicação entre executores auto-hospedados e {{ site.data.variables.product.prodname_dotcom }}

A máquina pode comunicar-se com {{ site.data.variables.product.prodname_actions }}. Para obter mais informações, consulte "[Comunicação entre os executores auto-hospedados e {{ site.data.variables.product.prodname_dotcom }}](#communication-between-self-hosted-runners-and-github)".

{% endif %}

### Comunicação entre executores auto-hospedados e {{ site.data.variables.product.product_name }}

As enquetes dos executores auto-hospedados {{ site.data.variables.product.product_name }} para recuperar atualizações do aplicativo e verificar se algum trabalho está na fila para processamento. O executor auto-hospedado usa uma _enquete longa_ HTTPS que abre uma conexão com {{ site.data.variables.product.product_name }} por 50 segundos e, se nenhuma resposta for recebida, o período de espera se encerra a uma nova enquete é criada. O aplicativo deve estar rodando na máquina para aceitar e executar trabalhos do {{ site.data.variables.product.prodname_actions }}.

{% if currentVersion == "free-pro-team@latest" %}

Você deve garantir que a máquina tenha acesso adequado à rede para comunicar-se com as {{ site.data.variables.product.prodname_dotcom }} URLs listadas abaixo.
...
Este não é um problema com executores hospedados no {{ site.data.variables.product.prodname_dotcom }}, pois cada executor hospedado no {{ site.data.variables.product.prodname_dotcom }} é sempre uma máquina virtual limpa e isolada, que é destruída no final da execução do trabalho.
