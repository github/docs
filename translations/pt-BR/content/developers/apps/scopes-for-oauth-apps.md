...
| **`site_admin`**         | Concede acesso de administrador aos pontos de extremidades da API de administração [{{ site.data.variables.product.prodname_ghe_server }}](/v3/enterprise-admin).{% endif %}
...
| &emsp;`security_events`  | Concede acesso de leitura e escrita a eventos de segurança na [API {{ site.data.variables.product.prodname_code_scanning }}](/v3/code-scanning).{% endif %}
...
| **`write:packages`**     | Concede acesso ao para fazer o upload ou publicação de um pacote no {{ site.data.variables.product.prodname_registry }}. Para obter mais informações, consulte "[Publicar um pacote](/github/managing-packages-with-github-packages/publishing-a-package)".                                                                                                                                        |
| **`read:packages`**      | Concede acesso ao download ou instalação de pacotes do {{ site.data.variables.product.prodname_registry }}. Para obter mais informações, consulte "[Instalando um pacote](/github/managing-packages-with-github-packages/installing-a-package)".                                                                                                                                                   |
| **`delete:packages`**    | Concede acesso para excluir pacotes de {{ site.data.variables.product.prodname_registry }}. Para obter mais informações, consulte "[Excluir pacotes](/github/managing-packages-with-github-packages/deleting-a-package)".{% endif %}
...
| **`fluxo de trabalho`**  | Concede a capacidade de adicionar e atualizar arquivos do fluxo de trabalho do {{ site.data.variables.product.prodname_actions }}. Os arquivos do fluxo de trabalho podem ser confirmados sem este escopo se o mesmo arquivo (com o mesmo caminho e conteúdo) existir em outro branch no mesmo repositório.{% endif %}
