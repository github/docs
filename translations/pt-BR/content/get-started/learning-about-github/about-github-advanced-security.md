...
- **Visão geral de segurança** – revise a configuração de segurança e os alertas de uma organização e identifique os repositórios com maior risco. Para obter mais informações, confira "[Sobre a visão geral de segurança](/code-security/security-overview/about-the-security-overview)".
{% endif %}
...
{%- ifversion ghes %} O administrador do site precisa habilitar o {% data variables.product.prodname_advanced_security %} para o {% data variables.product.product_location %} para que você possa usar esses recursos. Para obter mais informações, confira "[Como configurar recursos do Advanced Security](/admin/configuration/configuring-advanced-security-features)".
