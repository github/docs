...
intro: 'Você pode configurar o cliente do Docker para usar o {{ site.data.variables.product.prodname_registry }} para publicar e recuperar imagens do Docker.'
...
Ao instalar ou publicar uma imagem do docker, o {{ site.data.variables.product.prodname_registry }} não é compatível camadas externas, como imagens do Windows.

{% if currentVersion != "free-pro-team@latest" %}

Antes de poder usar o registro do Docker no {{ site.data.variables.product.prodname_registry }}, o administrador do site para {{ site.data.variables.product.product_location_enterprise }} deve habilitar o suporte do Docker e o isolamento do subdomínio para a sua instância. Para obter mais informações, consulte "[Gerenciar pacotes do GitHub para a sua empresa](/enterprise/admin/packages)".
...
Você pode efetuar a autenticação no {{ site.data.variables.product.prodname_registry }} usando o comando de login do `docker`.
...
To use this example login command, replace `USERNAME` with your {{ site.data.variables.product.prodname_dotcom }} username and `~/TOKEN.txt` with the file path to your personal access token for {{ site.data.variables.product.prodname_dotcom }}.
...
{% if currentVersion != "free-pro-team@latest" %} *HOSTNAME* com o nome de host de {{ site.data.variables.product.product_location_enterprise }},{% endif %} e *VERSÃO* com a versão do pacote no momento da criação.
  {% if currentVersion == "free-pro-team@latest" %}
  ```shell
...
{% if currentVersion != "free-pro-team@latest" %} *HOSTNAME* pelo o nome de host de {{ site.data.variables.product.product_location_enterprise }},{% endif %} e *PATH* para a imagem se não estiver no diretório de trabalho atual.
  {% if currentVersion == "free-pro-team@latest" %}
  ```shell
  ...
  ```
  {% endif %}
4. Publicar a imagem no {{ site.data.variables.product.prodname_registry }}.
  {% if currentVersion == "free-pro-team@latest" %}
  ```shell
...
# Faça push da imagem no {{ site.data.variables.product.prodname_registry }}
$ docker push docker.pkg.github.com/octocat/octo-app/monalisa:1.0
```
...
You can use the `docker pull` command to install a docker image from {{ site.data.variables.product.prodname_registry }}, replacing *OWNER* with the name of the user or organization account that owns the repository, *REPOSITORY* with the name of the repository containing your project, *IMAGE_NAME* with name of the package or image, *TAG_NAME* with tag for the image you want to install. {{ site.data.reusables.package_registry.lowercase-name-field }}
