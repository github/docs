...
intro: 'Você pode excluir uma versão de um pacote privado usando o GraphQL ou no {{ site.data.variables.product.product_name }}.'
...
Você só pode excluir uma versão específica de um pacote privado no {{ site.data.variables.product.product_name }} ou com a API do GraphQL. Para impedir que um pacote privado inteiro apareça no {{ site.data.variables.product.product_name }}, você deve primeiro excluir todas as versões do pacote.
...
### Excluir uma versão de um pacote privado no {{ site.data.variables.product.product_name }}
...
Use a mutação `deletePackageVersion` na API do GraphQL. Você deve usar um token com os escopos `read:packages`, `delete:packages` e `repo`. For more information about tokens, see "[About {{ site.data.variables.product.prodname_registry }}](/packages/publishing-and-managing-packages/about-github-packages#about-tokens)."
...
Você não pode excluir um pacote inteiro, mas se excluir todas as versões de um pacote, o pacote não será mais exibido em {{ site.data.variables.product.product_name }}
