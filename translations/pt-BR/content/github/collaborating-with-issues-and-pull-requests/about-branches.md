...
Também é possível usar um branch para publicar um site do {{ site.data.variables.product.prodname_pages }}. Para obter mais informações, consulte "[O que é {{ site.data.variables.product.prodname_dotcom }} Pages?](/articles/what-is-github-pages)"

Você deve ter acesso de gravação em um repositório para criar um branch, abrir uma pull request ou excluir e restaurar branches em uma pull request.  Para obter mais informações, consulte "[Permissões de acesso no {{ site.data.variables.product.product_name }}](/articles/access-permissions-on-github)".

### Sobre o branch-padrão

{{ site.data.reusables.branches.new-repo-default-branch }} O branch-padrão é o branch que {{ site.data.variables.product.prodname_dotcom }} exibe quando alguém visita o seu repositório. O branch-padrão é também o branch inicial que o Git verifica localmente quando alguém clona o repositório. {{ site.data.reusables.branches.default-branch-automatically-base-branch }}
...
No próximo diagrama, alguém fez merge da pull request para `feature1` no branch `mestre` , e eles excluíram o branch `feature1`. Como resultado, o {{ site.data.variables.product.prodname_dotcom }} redirecionou automaticamente a pull request para `feature2` para que seu branch base seja agora `mestre`.
...
- Se você usar o editor de conflitos do {{ site.data.variables.product.prodname_dotcom }} para corrigir conflitos para uma pull request que você criou de um branch protegido, o {{ site.data.variables.product.prodname_dotcom }}  ajuda você a criar um branch alternativo para o pull request, para que sua resolução de conflitos possa ser mesclada. Para obter mais informações, consulte "[Resolvendo um conflito de merge no {{ site.data.variables.product.prodname_dotcom }}](/github/collaborating-with-issues-and-pull-requests/resolving-a-merge-conflict-on-github)".{% endif %}
