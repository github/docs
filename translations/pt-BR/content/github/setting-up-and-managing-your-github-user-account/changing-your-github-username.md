...
intro: 'Você pode alterar seu nome de usuário do {{ site.data.variables.product.product_name }} a qualquer momento.'
redirect_from:
...
Se o nome de usuário não estiver qualificado para liberação e você não detém uma marca comercial para este nome de usuário, é possível escolher outro ou manter o atual. O {{ site.data.variables.contact.github_support }} não pode liberar o nome de usuário indisponível para você. Para obter mais informações, consulte "[Alterar nome de usuário](#changing-your-username)".{% endif %}
...
O {{ site.data.variables.product.product_name }} não pode configurar redirecionamentos para:
...
Após alteração do nome de usuário, o {{ site.data.variables.product.product_name }} redirecionará automaticamente as referências para os repositórios.
...
- "[{{ site.data.variables.product.prodname_dotcom }} Política de nome de usuário](/articles/github-username-policy)"{% endif %}
