...
intro: 'O gráfico de contribuições de perfil é um registro das contribuições que você fez em repositórios do {{ site.data.variables.product.product_name }}. As contribuições recebem registros de data e hora de acordo com o UTC (Coordinated Universal Time, Horário universal coordenado), e não com o fuso horário local. As contribuições só serão contabilizadas se atenderem a determinados critérios. Em alguns casos, pode ser necessário recriar o gráfico para que as contribuições sejam exibidas.'
redirect_from:
...
- O endereço de e-mail usado para os commits está associado à sua conta do {{ site.data.variables.product.product_name }}.
...
Para obter mais informações sobre sites de projeto, consulte "[Sobre {{ site.data.variables.product.prodname_pages }}](/github/working-with-github-pages/about-github-pages#types-of-github-pages-sites)".
...
Você precisará [adicionar o endereço de e-mail](/articles/adding-an-email-address-to-your-github-account) usado para o commit à sua conta do {{ site.data.variables.product.product_name }} caso ele não tenha sido adicionado ao seu perfil do {{ site.data.variables.product.product_name }}. O gráfico de contribuições será recriado automaticamente quando você adicionar o novo endereço.

{% warning %}

Não é possível adicionar endereços de e-mail genéricos, como `jane@computer.local`, a contas do {{ site.data.variables.product.product_name }}. Se você usar esse e-mail para os commits, eles não serão vinculados ao seu perfil do {{ site.data.variables.product.product_name }} e não serão exibidos no gráfico de contribuições.

{% endwarning %}

#### O commit não foi criado no branch `gh-pages` ou padrão

Os commits só são contados se forem feitos no branch-padrão ou no branch `gh-pages` (para repositórios com sites de projeto). Para obter mais informações, consulte "[Sobre {{ site.data.variables.product.prodname_pages }}](/github/working-with-github-pages/about-github-pages#types-of-github-pages-sites)".
...
- Para separar a bifurcação e transformá-la em um repositório autônomo no {{ site.data.variables.product.product_name }}, entre em contato com o {{ site.data.variables.contact.contact_support }}. Se a bifurcação tiver bifurcações próprias, informe o {{ site.data.variables.contact.github_support}} se as bifurcações devem mover-se com o repositório para uma nova rede ou se devem permanecer na rede atual. Para obter mais informações, consulte "[Sobre bifurcações](/articles/about-forks/)".
