...
- Seu nome de usuário {{ site.data.variables.product.product_name }}
...
- Uma conta de usuário {{ site.data.variables.product.product_name }} pessoal denominada **ghuser**
- Um repositório {{ site.data.variables.product.product_name }} denominado **repo.git**

{% endtip %}

1. [Crie um novo repositório em {{ site.data.variables.product.product_name }}](/articles/creating-a-new-repository). Você importará o repositório Git externo para este novo repositório.
2. Na linha de comando, faça um clone "vazio" do repositório usando a URL clone externo. Isso criará uma cópia integral dos dados, mas sem um diretório de trabalho para editar arquivos, e garantirá uma exportação limpa e recente de todos os dados antigos.
  ...
  ```
3. Faça o push do repositório clonado localmente em {{ site.data.variables.product.product_name }} usando a opção "mirror" (espelho), que assegura que todas as referências, como branches e tags, são copiadas para o repositório importado.
  ```shell
  $ cd <em>repo.git</em>
