...
intro: 'É possível definir ou atualizar determinados registros DNS e as configurações de repositório para apontar o domínio padrão do seu site do {{ site.data.variables.product.prodname_pages }} para um domínio personalizado.'
redirect_from:
...
Pessoas com permissões de administrador para um repositório podem configurar um domínio personalizado de um site do {{ site.data.variables.product.prodname_pages }}.

### Sobre a configuração de domínio personalizado

Lembre-se de adicionar o domínio personalizado ao seu site do {{ site.data.variables.product.prodname_pages }} antes de configurar o domínio personalizado com o provedor DNS. Se você configurar o domínio personalizado com o provedor DNS sem adicioná-lo ao {{ site.data.variables.product.product_name }}, outra pessoa conseguirá hospedar um site em um dos seus subdomínios.
...
Para configurar um domínio apex, como `example.com`, você deve configurar um arquivo _CNAME_ no repositório do {{ site.data.variables.product.prodname_pages }} e um registro `ALIAS`, `ANAME` ou `A` com o provedor DNS.
...
{{ site.data.reusables.repositories.sidebar-settings }}
{{ site.data.reusables.pages.save-custom-domain }}
5. Navegue até o provedor DNS e crie um registro `ALIAS`, `ANAME` ou `A`. {{ site.data.reusables.pages.contact-dns-provider }}
    - Para criar um registro `ALIAS` ou `ANAME`, aponte o domínio apex para o domínio padrão do seu site. {{ site.data.reusables.pages.default-domain-information }}
    - Para criar um registro `A`, aponte o domínio apex para os endereços IP do {{ site.data.variables.product.prodname_pages }}.
      ```shell
      185.199.108.153
      185.199.109.153
      185.199.110.153
      ...
      ```
{{ site.data.reusables.command_line.open_the_multi_os_terminal }}
6. Para confirmar que o registro DNS foi configurado corretamente, use o comando `dig`, substituindo _WW.EXAMPLE.COM_ pelo domínio apex. Confirme que os resultados correspondem aos endereços IP do {{ site.data.variables.product.prodname_pages }} acima.
  ```shell
