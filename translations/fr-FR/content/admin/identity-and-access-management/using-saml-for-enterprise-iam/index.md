---
title: Utilisation de SAML pour la gestion des identités et des accès d’entreprise
shortTitle: SAML for enterprise IAM
intro: 'Vous pouvez gérer de manière centralisée les {% ifversion ghes or ghae %}comptes et {% endif %}accéder {% ifversion ghes %}{% data variables.location.product_location %}{% elsif ghae %}à votre entreprise{% elsif ghec %}aux ressources de votre entreprise{% endif %} avec l’authentification unique SAML {% ifversion ghec or ghae %} et SCIM (System for Cross-domain Identity Management){% endif %}.'
versions:
  ghec: '*'
  ghes: '*'
  ghae: '*'
