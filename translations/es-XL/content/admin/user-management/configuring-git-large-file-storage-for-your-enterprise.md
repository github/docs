...
### Acerca de {{ site.data.variables.large_files.product_name_long }}

{{ site.data.reusables.enterprise_site_admin_settings.configuring-large-file-storage-short-description }} Puedes usar {{ site.data.variables.large_files.product_name_long }} con un repositorio único, con todos tus repositorios personales o de la organización, o con todos los repositorios de {{ site.data.variables.product.product_location_enterprise }}. Antes de poder habilitar {{ site.data.variables.large_files.product_name_short }} para repositorios u organizaciones específicas, debes habilitar {{ site.data.variables.large_files.product_name_short }} para tu aparato.

{{ site.data.reusables.large_files.storage_assets_location }}
{{ site.data.reusables.large_files.rejected_pushes }}

Para obtener más información, consulta "[Acerca de {{ site.data.variables.large_files.product_name_long }}](/articles/about-git-large-file-storage)", "[Control de versiones de archivos grandes](/enterprise/user/articles/versioning-large-files/)," y el sitio del proyecto [{{ site.data.variables.large_files.product_name_long }} ](https://git-lfs.github.com/).

{{ site.data.reusables.large_files.can-include-lfs-objects-archives }}

### Configurar {{ site.data.variables.large_files.product_name_long }} para tu aparato
...
4. Dentro del "acceso de {{ site.data.variables.large_files.product_name_short }}", usa el menú desplegable y haz clic en **Enabled (Habilitado)** o **Disabled (Inhabilitado)**. ![Acceso a LFS de Git](/assets/images/enterprise/site-admin-settings/git-lfs-admin-center.png)

### Configurar {{ site.data.variables.large_files.product_name_long }} para un repositorio individual
...
### Configurar {{ site.data.variables.large_files.product_name_long }} para cada repositorio que pertenezca a una cuenta de usuario u organización
...
3. Para mantener la misma configuración {{ site.data.variables.large_files.product_name_short }} para cada usuario, confirma un archivo `.lfsconfig` personalizado para el repositorio.
  ...
  ```
3. Migra cualquier activo {{ site.data.variables.large_files.product_name_short }} existente. Para obtener más información, consulta la sección "[Migrarse a un servidor diferente de {{ site.data.variables.large_files.product_name_long }}](#migrating-to-a-different-git-large-file-storage-server)".

### Migrar a un servidor de Git Large File Storage diferente

Antes de migrar a un servidor {{ site.data.variables.large_files.product_name_long }} diferente, debes configurar {{ site.data.variables.large_files.product_name_short }} para usar un servidor de un tercero. Para obtener más información, consulta la sección "[Configurar {{ site.data.variables.large_files.product_name_long }} para utilizar un servidor de terceros](#configuring-git-large-file-storage-to-use-a-third-party-server)".
