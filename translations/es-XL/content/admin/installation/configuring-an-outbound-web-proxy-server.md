...
intro: 'Un servidor proxy proporciona otro nivel de seguridad para {{ site.data.variables.product.product_location_enterprise }}.'
redirect_from:
...
Cuando se habilita un servidor proxy para {{ site.data.variables.product.product_location_enterprise }}, primero {{ site.data.variables.product.prodname_ghe_server }} envía mensajes fuera de banda a través del servidor proxy, a menos que el host de destino se agregue como una exclusión de servidor proxy HTTP. Los tipos de mensajes fuera de banda incluyen webhooks salientes, carga de paquetes y extracción de avatares heredados. La URL del servidor proxy es el protocolo, dominio o dirección IP más el número de puerto, por ejemplo `http://127.0.0.1:8123`.

{% note %}

**Nota:**  Para conectarte a {{ site.data.variables.product.product_location_enterprise }} para {{ site.data.variables.product.prodname_dotcom_the_website }}, tu configuración proxy debe permitir la conectividad a `github.com` y a `api.github.com`. Para obtener más información, consulta "[Conectarse a {{ site.data.variables.product.prodname_ghe_server }} para {{ site.data.variables.product.prodname_dotcom_the_website }}](/enterprise/{{ currentVersion }}/admin/guides/developer-workflow/connecting-github-enterprise-server-to-github-com)."
