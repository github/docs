...
intro: 'Para subir tu proyecto a {{ site.data.variables.product.product_location }}, deberás crear un repositorio donde alojarlo.'
versions:
  free-pro-team: '*'
  enterprise-server: '*'
---

Puedes almacenar distintos proyectos en los repositorios de {{ site.data.variables.product.product_name }}, incluso proyectos de código abierto. Con [proyectos de código abierto](http://opensource.org/about), puedes compartir el código para hacer que el software funcione mejor y sea más confiable.

{% note %}

**Nota:** Puedes crear repositorios públicos para un proyecto de código abierto. Cuando crees un repositorio público, asegúrate de incluir un [archivo de licencia](http://choosealicense.com/) que determine cómo deseas que se comparta tu proyecto con otros usuarios. {{ site.data.reusables.open-source.open-source-guide-repositories }}{{ site.data.reusables.open-source.open-source-learning-lab }}
...
3. También puedes agregar una descripción de tu repositorio. Por ejemplo, "Mi primer repositorio en {{ site.data.variables.product.product_name }}". ![Campo para ingresar una descripción para el repositorio](/assets/images/help/repository/create-repository-desc.png)
...
¡Felicitaciones! Has creado un repositorio, además de un archivo *README*, y has creado tu primera confirmación en {{ site.data.variables.product.product_location }}. ¿Qué deseas hacer ahora?
