...
Todo gist es un repositorio Git, lo que significa que se puede bifurcar y clonar. Si has iniciado sesión en {{ site.data.variables.product.product_name }}, cuando crees un gist, este se asociará con tu cuenta y lo verás en tu lista de gists cuando te dirijas a tu {{ site.data.variables.gists.gist_homepage }}.

Los gists pueden ser públicos o secretos. Los gists públicos se muestran en {{ site.data.variables.gists.discover_url }}, donde las personas pueden explorar los nuevos gists a medida que estos se crean. También se los puede buscar, para que puedas usarlos si deseas que otras personas encuentren tu trabajo y lo vean. {{ site.data.reusables.gist.cannot-convert-public-gists-to-secret }}

Los gists secretos no se muestran en {{ site.data.variables.gists.discover_url }}{% if currentVersion != "free-pro-team@latest" %}, {% endif %} y no se pueden buscar. {{ site.data.reusables.gist.cannot-convert-public-gists-to-secret }}Los gists secretos no son privados. Si envías la URL de un gist secreto a un amigo, este podrá verlo. Sin embargo, si alguien que no conoces descubre la URL, también podrá ver tu gist. Si deseas mantener tu código a salvo de las miradas curiosas, puedes optar por [crear un repositorio privado](/articles/creating-a-new-repository) en lugar de un gist.
...
Puedes descubrir gists creados por otros usuarios si te diriges a la {{ site.data.variables.gists.gist_homepage }} y haces clic en **Todos los gists**. Esto te llevará a una página en la que aparecen todos los gists clasificados y presentados por fecha de creación o actualización. También puedes buscar los gists por idioma con {{ site.data.variables.gists.gist_search_url }}. La búsqueda de gists usa la misma sintaxis de búsqueda que la [búsqueda de código](/articles/searching-code).
...
Git admite la asignación de archivos GeoJSON. Estas asignaciones se muestran como gists insertos, para que las asignaciones se puedan compartir e insertar fácilmente. Para obtener más información, consulta "[Asignar archivos GeoJSON en {{ site.data.variables.product.product_name }}](/articles/mapping-geojson-files-on-github)".
...
1. Inicia sesión en {{ site.data.variables.product.product_name }}.
2. Dirígete a tu {{ site.data.variables.gists.gist_homepage }}.
...
5. Realiza una de las siguientes acciones:
  ...
  **Nota:{{ site.data.reusables.gist.cannot-convert-public-gists-to-secret }}**
