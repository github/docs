...
intro: 'Puedes utilizar el inicio de sesión único (SSO, por sus siglas en inglés) del Lenguaje de Marcado para Confirmaciones (SAML, por sus siglas en inglés) y la Adminsitración de Identidad a Través de Dominios (SCIM, por sus siglas en inglés) con Okta para adminsitrar automáticamente el acceso a tu cuenta empresarial en {{ site.data.variables.product.product_name }}.'
...
Puedes controlar el acceso a tu cuenta empresarial en {{ site.data.variables.product.product_name }} y en otras aplicaciones web desde una interface central si configuras dicha cuenta para que utilice el SSO de SAML y SCIM con Okta, un proveedor de identidad (IdP).
...
1. Da clic en "{{ site.data.variables.product.prodname_ghe_cloud }} - Cuentas Empresariales".
...
1. A la derecha de "Empresas de {{ site.data.variables.product.prodname_dotcom }}", teclea el nombre de tu cuenta empresarial. Por ejemplo, si la URL de tu cuenta empresarial es `https://github.com/enterprises/octo-corp`, teclea `octo-corp`. ![Campo de Github Enterprises](/assets/images/help/saml/okta-github-enterprises.png)
...
1. Asigna la aplicación que creaste para tu cuenta empresarial a cada grupo. {{ site.data.variables.product.prodname_dotcom }} recibirá todos los datos de los `groups` para cada usuario.
...
Para configurar el aprovisionamiento de usuarios con SCIM en Okta, debes autorizar una aplicación de OAuth para que cree un token que Okta pueda utilizar para autenticarse en {{ site.data.variables.product.product_name }} a tu nombre. Okta creó la aplicación okta-oauth en asociación con {{ site.data.variables.product.prodname_dotcom }}.
...
1. Da clic en **Autenticarse con GitHub Enterprise Cloud - Cuentas Empresariales**. ![Botón para autenticarse con {{ site.data.variables.product.prodname_dotcom }}](/assets/images/help/business-accounts/authenticate-with-github-button.png)
...
1. Si ves la opción **Aprovisionar usuarios**, los usuarios que fuesen miembros de un grupo de Okta antes de que agregaras un grupo de subida para este grupo no se han aprovisionado. Para enviar datos de SCIM de estos usuarios a {{ site.data.variables.product.product_name }}, da clic en **Aprovisionar usuarios**.
