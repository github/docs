...
- Tu nombre de usuario {{ site.data.variables.product.product_name }}
...
- Una cuenta de usuario personal {{ site.data.variables.product.product_name }} llamada **ghuser**
- Un repositorio {{ site.data.variables.product.product_name }} llamado **repo.git**

{% endtip %}

1. [Crear un repositorio nuevo en {{ site.data.variables.product.product_name }}](/articles/creating-a-new-repository). Importarás tu repositorio de Git externo a este repositorio nuevo.
2. En la línea de comando, haz un clon "en blanco" del repositorio usando la URL del clon externo. Esto crea una copia completa de los datos, pero sin un directorio de trabajo para editar archivos, y asegura una exportación limpia y nueva de todos los datos antiguos.
  ...
  ```
3. Sube el repositorio clonado de forma local a {{ site.data.variables.product.product_name }} usando la opción "espejo", que asegura que todas las referencias, como ramas y etiquetas, se copien en el repositorio importado.
  ```shell
  $ cd <em>repo.git</em>
