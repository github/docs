...
| Nombre          | Tipo        | Descripción                                                                                                                                                            |
| --------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `client_id`     | `secuencia` | **Requerido.** La ID de cliente que recibiste de {{ site.data.variables.product.product_name }} para tu {{ site.data.variables.product.prodname_github_app }}.       |
| `client_secret` | `secuencia` | **Requerido.** El secreto del cliente que recibiste de {{ site.data.variables.product.product_name }} para tu {{ site.data.variables.product.prodname_github_app }}. |
...
2. La app pide al usuario ingresar un código de verificación de usuario en {{ site.data.variables.product.device_authorization_url }}.
...
| Nombre      | Tipo        | Descripción                                                                                                  |
| ----------- | ----------- | ------------------------------------------------------------------------------------------------------------ |
| `client_id` | `secuencia` | **Requerido.** La ID de cliente que recibiste de {{ site.data.variables.product.product_name }} para tu app. |
...
| `verification_uri` | `secuencia` | La URL de verificación en donde los usuarios necesitan ingresar el `user_code`: {{ site.data.variables.product.device_authorization_url }}.                                                                                                                                                                                                                                                                                                                                                                |
...
| `interval`         | `número`    | La cantidad mínima de segundos que deben transcurrir antes de que puedas hacer una soliciud de token de acceso nueva (`POST {{ site.data.variables.product.oauth_host_code }}/login/oauth/access_token`) para completar la autorización del dispositivo. Por ejemplo, si el intervalo es 5, entonces no puedes hacer una solicitud nueva hasta que hayan transcurrido 5 segudos. Si haces más de una solicitud en estos 5 segundos, entonces lelgarás al límite de tasa y recibirás un error de `slow_down`. |

#### Paso 2: Indicar al usuario ingresar el código de usuario en un buscador

Tu dispositivo mostrará el código de verificación de usuario y pedirá al usuario ingresar el código en la {{ site.data.variables.product.device_authorization_url }}.
...
Tu app hará solicitudes de autorización de dispositivo que sondean a `POST {{ site.data.variables.product.oauth_host_code }}/login/oauth/access_token`, hasta que los códigos de dispositivo y de usuario caduquen o hasta que el usuario haya autorizado la app con éxito con un código de usuario válido. La app debe usar el `interval` de sondeo mínimo que se ha recuperado en el paso 1 para evitar los errores de límite de tasa. Para obtener más información, consulta la sección "[Límites de tasa para el flujo del dispositivo](#rate-limits-for-the-device-flow)".

El usuario debe ingresar un código válido dentro de los 15 minutos (o 900 segundos) siguientes. Después de transcurridos estos 15 minutos, necesitarás solicitar un código de autorización de dispositivo nuevo con `POST {{ site.data.variables.product.oauth_host_code }}/login/device/code`.
...
| Nombre        | Tipo        | Descripción                                                                                                                                                           |
| ------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `client_id`   | `secuencia` | **Requerido.** La ID de cliente que recibiste de {{ site.data.variables.product.product_name }} para tu {{ site.data.variables.product.prodname_oauth_app }}.       |
| `device_code` | `secuencia` | **Requerido.** El código de verificación del dispositivo que recibiste de la solicitud de `POST {{ site.data.variables.product.oauth_host_code }}/login/device/code`. |
...
Si realizas más de una solicitud de acceso con token (`POST {{ site.data.variables.product.oauth_host_code }}/login/oauth/access_token`) dentro del marco de tiempo mínimo requerido entre solicitudes (o `interval`), alcanzarás el límite de tasa y recibirás una respuesta de error de `slow_down`. La respuesta de error `slow_down` agrega 5 segundos al último `interval`. Para obtener más información, consulta los [Errores para el flujo del dispositivo](#errors-for-the-device-flow).

#### Códigos de error para el flujo del dispositivo

| Código de error                | Descripción                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `authorization_pending`        | Este error ocurre cuando la solicitud de autorización se encuentra pendiente y el usuario no ha ingresado el código de usuario aún. Se espera que la app siga sondeando la solicitud de  `POST {{ site.data.variables.product.oauth_host_code }}/login/oauth/access_token` sin exceder el [`interval`](#response-parameters), lo cual requiere una cantidad mínima de segundos entre cada solicitud.                                                                                                                                                                                      |
| `slow_down`                    | Cuando recibes el error de `slow_down`, se agregan 5 segundos extra al `interval` mínimo o al marco de tiempo requerido entre tus solicitudes utilizando `POST {{ site.data.variables.product.oauth_host_code }}/login/oauth/access_token`. Por ejemplo, si el intervalo de inicio requirió por lo menos 5 segundos entre solicitudes y obtienes una respuesta de error de `slow_down`, ahora necesitarás esperar por lo menos 10 segundos antes de que hagas una solicitud nueva para un token de acceso de OAuth. La respuesta de error incluye el nuevo `interval` que debes utilizar. |
...
| `unsupported_grant_type`       | El tipo de otorgamiento debe ser `urn:ietf:params:oauth:grant-type:device_code` y se debe incluir como un parámetro de entrada cuando sondeas la solicitud de token de OAuth `POST {{ site.data.variables.product.oauth_host_code }}/login/oauth/access_token`.                                                                                                                                                                                                                                                                                                                           |
