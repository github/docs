...
您可以为每个安装目标{% ifversion ghes or ghae %}（{% data variables.product.prodname_ghe_server %}实例、特定组织或特定仓库）{% else %}（特定组织或特定仓库）{% endif %}上的每个事件创建最多 {% ifversion ghes or ghae %}250{% else %}20{% endif %} 个 web 挂钩。
