...
### 步骤 1.1. Updating app permissions
...
太好了！ Your app has permission to do the tasks you want it to do. Now you can add the code to handle the events.

### 步骤 1.2. Adding event handling
...
太好了！ You've told GitHub to create a check run. You can see the check run status is set to `queued` next to a yellow icon. Next, you'll want to wait for GitHub to create the check run and update its status.

### 步骤 1.4. Updating a check run
...
### 步骤 2.3. Running RuboCop

太好了！ You're cloning the repository and creating check runs using your CI server. Now you'll get into the nitty gritty details of the [RuboCop linter](https://rubocop.readthedocs.io/en/latest/basic_usage/#rubocop-as-a-code-style-checker) and [Checks API annotations](/v3/checks/runs/#create-a-check-run).
...
### 步骤 2.4. Collecting RuboCop errors
...
### 步骤 2.5. Updating the check run with CI test results
...
### 步骤 2.6. Automatically fixing RuboCop errors
...
搞定！ The code you have written now completes your Checks API CI server. 💪 Restart your `template_server.rb` server again and create a new pull request:
