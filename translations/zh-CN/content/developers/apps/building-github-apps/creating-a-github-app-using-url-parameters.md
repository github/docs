...
## {% data variables.product.prodname_github_app %} 配置参数
 ...
 | `webhook_active`                                   | `布尔值`   | Set to `false` to disable webhook. Webhook is enabled by default.                                                                                                                                                                                                                                                               |
 ...
 | {% endif %}`events`                                | `字符串数组` | Web 挂钩事件. 某些 web 挂钩事件需要您对资源有 `read` 或 `write` 权限，才能在注册新 {% data variables.product.prodname_github_app %} 时选择事件。 有关可用事件及其所需权限，请参阅“[{% data variables.product.prodname_github_app %} web 挂钩事件](#github-app-webhook-events)”一节。 您可以在查询字符串中选择多个事件。 For example, `events[]=public&events[]=label`.{% ifversion ghes < 3.4 %}
 | `域`                                                | `字符串`   | The URL of a content reference.{% endif %}
