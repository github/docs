---
title: 错误：代理承认没有签署
intro: '在极少数情况下，在 Linux 上通过 SSH 连接 {{ site.data.variables.product.product_name }} 会产生错误“Agent admitted failure to sign using the key”（代理承认没有使用密钥签署）。 请遵循以下步骤解决此问题。'
redirect_from:
...
在 Linux 上尝试将通过 SSH 连接到 {{ site.data.variables.product.product_location }} 时，可能在终端上看到以下信息：
