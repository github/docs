...
{{ site.data.reusables.two_fa.about-recovery-codes }} 也可以在启用双重身份验证后随时下载恢复代码。
...
通过异地恢复帐户功能，您可以为 {{ site.data.variables.product.product_name }} 帐户添加额外的安全因素，以应对无法访问双重身份验证方法或恢复代码的情况。

异地恢复帐户功能允许您将 {{ site.data.variables.product.product_name }} 帐户与您的 Facebook 帐户相关联。 您可以使用 Facebook 以_帐户恢复令牌_的形式存储 {{ site.data.variables.product.product_name }} 帐户的身份验证凭据。

如果因无法访问双重身份验证方法或恢复代码而无法访问 {{ site.data.variables.product.product_name }} 帐户，您可以从恢复提供程序取回您的帐户恢复令牌，以帮助证明您是您的 {{ site.data.variables.product.product_name }} 帐户的所有者。

取回令牌后，{{ site.data.variables.contact.contact_support }} 可能会对帐户禁用双重身份验证。 然后，您可以提供或重置密码以恢复帐户访问。
...
1. 登录您的 Facebook 帐户，然后返回到 {{ site.data.variables.product.product_name }}。
{{ site.data.reusables.user_settings.access_settings }}
{{ site.data.reusables.user_settings.security }}
