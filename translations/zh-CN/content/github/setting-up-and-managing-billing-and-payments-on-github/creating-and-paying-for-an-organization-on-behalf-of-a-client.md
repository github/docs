---
title: 代客户创建并支付组织
intro: '您可以代客户创建并支付 {{ site.data.variables.product.prodname_dotcom }} 组织。'
redirect_from:
...
在开始之前，请确保您知道：
- 将成为组织所有者的客户的 {{ site.data.variables.product.prodname_dotcom }} 用户名
- 客户希望组织使用的名称
- 您希望收到收据的电子邮件地址
- 客户希望购买的[产品](/articles/github-s-products)
- 客户希望您为组织购买的[付费席位](/articles/about-per-user-pricing/)数

### 第 1 步：创建您的个人 {{ site.data.variables.product.prodname_dotcom }} 帐户。

您将使用您的个人帐户来设置组织。 以后续订或更改客户的订阅时也需要登录此帐户。

如果您已有 {{ site.data.variables.product.prodname_dotcom }} 用户帐户，请跳至 [第 2 步](#step-2-create-the-organization)。
...
3. 为您的个人帐户选择 {{ site.data.variables.product.prodname_free_user }}。
...
3. 在“Choose a plan（选择计划）”下，单击 **选择 {{ site.data.variables.product.prodname_free_team }}**。 您将在下一步升级组织。
...
{{ site.data.reusables.dotcom_billing.choose_org_plan }} （在下一步可向组织添加更多席位。）
...
5. 键入客户的 {{ site.data.variables.product.prodname_dotcom }} 用户名，然后按 **Enter** 键。 ![键入客户用户名的字段](/assets/images/help/organizations/org-invite-modal.png)
...
2. 如果要从组织中删除您组织的信用卡以便不再从该卡付费，请联系 {{ site.data.variables.contact.contact_support }}。
