...
您可以查看待处理成员和外部协作者的数量。 您可以按{% if currentVersion == "free-pro-team@latest" %}部署（{% data variables.product.prodname_ghe_cloud %} 或 {% data variables.product.prodname_ghe_server %}）、{% endif %}角色{% if currentVersion == "free-pro-team@latest" %}和{% elsif currentVersion == "github-ae@latest" %}或{% endif %}组织过滤成员列表。 您可以按协作者具有访问权限的仓库的可见性来过滤外部协作者列表。 您可以通过搜索其用户名或显示名称查找特定人员。
