...
**提示：**只有组织所有者才能重命名组织。 {{ site.data.reusables.organizations.new-org-permissions-more-info }}
...
- {{ site.data.variables.product.prodname_dotcom }} 将引用自动重定向到您的仓库。  指向您组织现有**仓库**的 Web 链接将继续有效。 启动更改后，可能需要几分钟时间才能完成。
...
更改组织的名称后：
- 指向以前组织资料页面的链接（例如 `https://{{ site.data.variables.command_line.backticks }}/previousorgname`）将返回 404 错误。 我们建议您更新其他站点指向组织的链接{% if currentVersion == "free-pro-team@latest" %}，例如 LinkedIn 或 Twitter 个人资料{% endif %}。
