...
在 {{ site.data.variables.product.product_name }} 上，您可以按以下方式查看仓库的提交历史记录：
...
对于单个文件的提交历史记录，{{ site.data.variables.product.product_name }} 明确遵循此简单策略。 它会移除对最终结果没有作用的提交，简化历史记录。 例如，如果侧分支做了更改，然后还原，则该提交不会显示在分支历史记录中。 这可提高分支审查的效率，因为您只看到影响文件的提交。

此截断视图可能并不总是包含您需要的信息。 如果要查看整个历史记录，{{ site.data.variables.product.product_name }} 会在仓库的提交页面提供信息更多的视图。
