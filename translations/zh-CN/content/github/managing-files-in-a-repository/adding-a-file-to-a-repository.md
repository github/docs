---
title: 添加文件到仓库
intro: '您可以上传并提交现有文件到 {{ site.data.variables.product.product_name }} 仓库。 将文件拖放到文件树中的任何目录，或者从仓库的主页面上传文件。'
redirect_from:
...
通过浏览器添加到仓库的文件大小限于每个文件 {{ site.data.variables.large_files.max_github_browser_size }}。 较大的文件可通过命令行添加，最大每个 {{ site.data.variables.large_files.max_github_size }}。 更多信息请参阅“[使用命令行添加文件到仓库](/articles/adding-a-file-to-a-repository-using-the-command-line)”。

{% tip %}

**提示：**
- 您可以同时将多个文件上传到 {{ site.data.variables.product.product_name }}。
