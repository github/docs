...
intro: '您的个人资料贡献图是您对 {{ site.data.variables.product.product_name }} 仓库所做贡献的记录。 贡献按照协调世界时 (UTC) 而不是您当地的时区加时间戳。 只有在满足特定标准时才会计算贡献。 在某些情况下，我们可能需要重建您的图表才能显示贡献。'
redirect_from:
...
- 用于提交的电子邮件地址与您的 {{ site.data.variables.product.product_name }} 帐户关联。
- 提交在独立的仓库而不是复刻中进行。
- 提交在以下位置进行：
...
如果用于提交的电子邮件地址尚未添加到 {{ site.data.variables.product.product_name }} 个人资料，您必须[添加电子邮件地址](/articles/adding-an-email-address-to-your-github-account)到 {{ site.data.variables.product.product_name }} 帐户。 您的贡献图将在添加新地址后自动重建。

{% warning %}

通用电子邮件地址（例如 `jane@computer.local`）无法添加到 {{ site.data.variables.product.product_name }} 帐户。 如果为您的提交使用这类电子邮件，则提交不会链接到您的 {{ site.data.variables.product.product_name }} 个人资料，并且不会在您的贡献图中显示。
...
- 要分离复刻并将其变为 {{ site.data.variables.product.product_name }} 上独立的仓库，请联系 {{ site.data.variables.contact.contact_support }}。 如果该复刻有自己的复刻，让 {{ site.data.variables.contact.github_support}} 了解这些复刻是随您的仓库移入新网络还是留在当前网络中。 更多信息请参阅“[关于复刻](/articles/about-forks/)”。
