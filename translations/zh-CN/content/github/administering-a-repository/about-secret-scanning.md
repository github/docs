...
{% data variables.product.company_short %} 合作提供其用于扫描的密码格式。{% if currentVersion == "free-pro-team@latest" %} 更多信息请参阅“[密码扫描](/developers/overview/secret-scanning)”。
{% endif %}
...
当 {% data variables.product.prodname_secret_scanning %} 在{% if currentVersion == "free-pro-team@latest" %}私有{% endif %}仓库中检测到密码时，{% data variables.product.prodname_dotcom %} 会发送警报。
...
要监控
私有仓库或组织的 {% data variables.product.prodname_secret_scanning %} 结果，您可以使用 {% data variables.product.prodname_secret_scanning %} API。 有关 API 端点的更多信息，请参阅“[{% data variables.product.prodname_secret_scanning_caps %}](/rest/reference/secret-scanning)”。{% endif %}
