...
在 {{ site.data.variables.product.product_name }} 上还原拉取请求会创建一个新拉取请求。其中包含原始合并拉取请求中一个还原的合并提交。
...
- 还原拉取请求导致合并冲突
- 原始拉取请求最初在 {{ site.data.variables.product.product_name }} 上未合并。 例如，有人可能已在命令行上使用快进合并来合并拉取请求。
