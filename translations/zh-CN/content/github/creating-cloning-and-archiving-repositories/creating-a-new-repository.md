...
6. {% if currentVersion == "free-pro-team@latest" or currentVersion ver_gt "enterprise-server@2.17" %}如果没有使用模板，{% else %}{% endif %}您可以使用许多可选项预填充仓库。 如果要将现有仓库导入 {{ site.data.variables.product.product_name }}，请不要选择上述任何选项，否则可能会导致合并冲突。 您可以{% if currentVersion == "free-pro-team@latest" or currentVersion ver_gt "enterprise-server@2.15" %}通过用户界面添加或创建新文件，或者{% endif %}选择稍后使用命令行添加新文件。 更多信息请参阅“[使用命令行导入 Git 仓库](/articles/importing-a-git-repository-using-the-command-line/)”、“[使用命令行将文件添加到仓库](/articles/adding-a-file-to-a-repository-using-the-command-line)”和“[解决合并冲突](/articles/addressing-merge-conflicts/)”。
