...
在复制仓库和和推送到仓库的新副本或_镜像_之前，必须在 {{ site.data.variables.product.product_location }} 上[创建新仓库](/articles/creating-a-new-repository)。 在以下示例中，`exampleuser/new-repository` 或 `exampleuser/mirrored` 是镜像。
...
### 镜像包含 {{ site.data.variables.large_files.product_name_long }} 对象的仓库。
...
3. 导航到刚克隆的仓库。
  ...
  $ cd <em>old-repository</em>.git
  ```
4. 拉取仓库的 {{ site.data.variables.large_files.product_name_long }} 对象。
  ```shell
  $ git lfs fetch --all
  ```
5. 镜像推送至新仓库。
  ...
  $ git push --mirror https://{{ site.data.variables.command_line.codeblock }}/<em>exampleuser</em>/<em>new-repository</em>.git
  ```
6. 将仓库的 {{ site.data.variables.large_files.product_name_long }} 对象推送至镜像。
  ```shell
