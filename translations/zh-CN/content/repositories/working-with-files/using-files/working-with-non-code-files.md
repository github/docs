...
查看时，任何提交到 {% ifversion ghae %}{% data variables.product.product_name %}{% else %}{% data variables.product.product_location %}{% endif %} 上存储库的 .csv 或 .tsv 文件都会自动呈现为交互式表格，包括标题和行号。 默认情况下，我们始终假设第一行是标题行。
...
当在 {% data variables.product.product_location %} 上添加扩展名为 .ipynb 的 Jupyter Notebook 或 IPython Notebook 文件时，它们将在你的存储库中呈现为静态 HTML 文件。
