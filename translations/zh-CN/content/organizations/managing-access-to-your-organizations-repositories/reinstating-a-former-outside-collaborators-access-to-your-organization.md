...
当外部协作者对组织私有仓库的权限被删除时，该用户的访问权限和设置将保存三个月。 如果您在该时间范围内将用户{% ifversion fpt %}邀请{% else %}添加{% endif %}回组织，则可以恢复该用户的权限。
...
**提示**：
 ...
 - 恢复 {% data variables.product.product_location %} 上的成员流量可以使用术语“成员”来描述恢复外部协作者，但如果您恢复此人员并保留其以前的权限，则他们将只拥有以前的[外部协作者权限](/articles/permission-levels-for-an-organization/#outside-collaborators)。{% ifversion fpt %}
