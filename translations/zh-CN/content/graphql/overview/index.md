---
title: 概览
intro: '了解 {{ site.data.variables.product.prodname_dotcom }} GraphQL API、即将发生的变更预览、重大变更和限制。 您也可以使用 GraphQL Explorer 与真实 {{ site.data.variables.product.prodname_dotcom }} 数据的 API 交互。'
versions:
  free-pro-team: '*'
  enterprise-server: '*'
---

{% link_in_list /about-the-graphql-api %}

{% link_in_list /public-schema %}

{% link_in_list /schema-previews %}

{% link_in_list /breaking-changes %}

{% link_in_list /changelog %}

{% link_in_list /explorer %}

{% link_in_list /resource-limitations %}
