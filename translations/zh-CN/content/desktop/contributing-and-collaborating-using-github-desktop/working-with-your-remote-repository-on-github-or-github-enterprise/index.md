---
title: 在 GitHub 或 GitHub Enterprise 上使用远程仓库
intro: '在本地更改项目时，可以通过远程仓库持续更新它们。 在 Git 中，远程是存储代码的服务器。 在您的情况中，该服务器是 {% data variables.product.prodname_dotcom %} 或 {% data variables.product.prodname_enterprise %} 上的仓库。'
redirect_from:
  - /desktop/contributing-to-projects/working-with-your-remote-repository-on-github-or-github-enterprise
versions:
  fpt: '*'
children:
  - /creating-an-issue-or-pull-request
  - /viewing-a-pull-request-in-github-desktop
  - /viewing-and-re-running-checks-in-github-desktop
  - /configuring-notifications-in-github-desktop
  - /changing-the-remote-url-for-a-repository-from-github-desktop
shortTitle: Work with your remote repo
ms.openlocfilehash: 60c2f41546ff085ea8f8d56142da6a3c7a81ebae
ms.sourcegitcommit: fcf3546b7cc208155fb8acdf68b81be28afc3d2d
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/10/2022
ms.locfileid: '145099261'
---

