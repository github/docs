...
{% endif %}
  ...
  **提示：**不关注仓库，而是考虑仅{% ifversion fpt or ghes > 3.0 or ghae-next or ghec %}当 {% data reusables.notifications-v2.custom-notification-types %}（如果已对仓库启用）或这些选项的任何组合有更新、{% else %}仓库中有发布{% endif %}或完全取消关注仓库时才接收通知。
