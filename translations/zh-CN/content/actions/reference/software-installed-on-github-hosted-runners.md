...
intro: '本文链接到 {{ site.data.variables.product.prodname_dotcom }} 托管的虚拟环境中可用包和工具的参考。'
...
{{ site.data.variables.product.prodname_dotcom }} 托管的运行器中包含的工具每周更新。 有关每个运行器操作系统包含的工具最新列表，请参阅以下链接：
...
{{ site.data.variables.product.prodname_dotcom }} 托管的运行器除了上述参考中列出的包之外，还包括操作系统的默认内置工具。 例如，Ubuntu 和 macOS 运行器除了其他默认工具之外，还包括 `grep`、`find` 和 `which`。
