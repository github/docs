...
| metadata      | 读取        | 读取 | 读取 |
| packages      | 读/写  | 无 | 读取 |
{%- ifversion fpt or ghec or ghes > 3.2 or ghae-issue-6187 %} | 页面         | 读/写  | 无 | 读取 | {%- endif %} | 拉取请求 | 读/写  | 无 | 读取 | | 存储库项目 | 读/写 | 无 | 读取 | | 安全事件     | 读/写 | 无 | 读取 | | 状态      | 读/写  | 无 | 读取 |
...
如果你需要的令牌需要 `GITHUB_TOKEN` 中未提供的权限，可以创建个人访问令牌并将其设置为存储库中的机密：

1. 使用或创建具有该仓库适当权限的令牌。 有关详细信息，请参阅“[创建个人访问令牌](/github/authenticating-to-github/creating-a-personal-access-token)”。
