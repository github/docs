...
在此工作流程中，您可以使用 `setup-java` 操作。 此操作将 JDK 的给定版本安装到 `PATH`，但同时会配置 Maven _settings.xml_ 以发布包。 默认情况下，设置文件将配置用于 {{ site.data.variables.product.prodname_registry }}，但可以将其配置为部署到另一个包注册表，如 Maven 中心仓库。 如果您已经在 _pom.xml_ 配置分配管理仓库，则可在 `setup-java` 操作调用期间指定该 `id`。
...
在此工作流程中，您可以使用 `setup-java` 操作。 此操作将给定版本的 JDK 安装到 `PATH`，并且设置 Maven _settings.xml_ 以将包发布到 {{ site.data.variables.product.prodname_registry }}。 生成的 _settings.xml_ 定义使用 `github` 的 `id` 向服务器验证，使用 `GITHUB_ACTOR` 环境变量作为用户名，`GITHUB_TOKEN` 环境变量作为密码。
...
对于基于 Maven的项目，您可以通过在 _pom.xml_ 文件中创建分发仓库来使用这些设置，该文件以 `github` 的 `id` 指向 {{ site.data.variables.product.prodname_registry }} 端点。

例如，如果组织名为“octocat”且仓库名为“hello-world”，则 _pom.xml_ 中的 {{ site.data.variables.product.prodname_registry }} 配置看起来类似于以下示例。
...
通过此配置，您可以创建一个工作流程，以使用自动生成的 _settings.xml_ 将包发布到 {{ site.data.variables.product.prodname_registry }}。
...
您可以使用每个注册表的 `setup-node` 操作将包发布到 Maven 中心仓库和 {{ site.data.variables.product.prodname_registry }}。

确保 _pom.xml_ 文件包含用于 {{ site.data.variables.product.prodname_dotcom }} 仓库和 Maven 中心仓库提供商的分发管理仓库。 例如，如果您通过 OSSRH 托管项目部署到中心仓库，您可能想通过将 `id` 设置为 `ossrh` 在分发管理仓库中指定它，并且想通过将 `id` 设置为 `github` 在分发管理仓库中指定 {{ site.data.variables.product.prodname_registry }}。
...
1. {{ site.data.reusables.github-actions.publish-to-maven-workflow-step }}
1. 第二次调用 `setup-java`。 这将自动为 {{ site.data.variables.product.prodname_registry }} 配置 Maven _settings.xml_ 文件。
1. {{ site.data.reusables.github-actions.publish-to-packages-workflow-step }}
