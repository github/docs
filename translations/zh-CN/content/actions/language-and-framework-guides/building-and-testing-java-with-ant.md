...
{{ site.data.variables.product.prodname_dotcom }} 托管的运行器有工具缓存预安装的软件，包括 Java Development Kits (JDKs) 和 Ant。 有关软件以及 JDK 和 Ant 预安装版本的列表，请参阅 [{{ site.data.variables.product.prodname_dotcom }} 自托管运行器上安装的软件](/actions/automating-your-workflow-with-github-actions/software-installed-on-github-hosted-runners)。
...
{{ site.data.variables.product.prodname_dotcom }} 提供有 Ant 工作流程模板，应该适用于大多数基于 Ant 的 Java 项目。 更多信息请参阅 [Ant 工作流程模板](https://github.com/actions/starter-workflows/blob/master/ci/ant.yml)。
