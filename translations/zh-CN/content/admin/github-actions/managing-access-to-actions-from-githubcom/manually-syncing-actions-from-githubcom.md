...
注意：`actions-sync` 工具适用于未启用 {% data variables.product.prodname_github_connect %} 的系统。 如果在启用了 {% data variables.product.prodname_github_connect %} 的系统上运行该工具，则可能会看到错误 `The repository <repo_name> has been retired and cannot be reused`。 这表示工作流已直接在 {% data variables.product.prodname_dotcom_the_website %} 上使用了该操作，并且命名空间已在 {% data variables.location.product_location %}上停用。 有关详细信息，请参阅“[自动停用在 {% data variables.product.prodname_dotcom_the_website%} 上访问的操作的命名空间](/admin/github-actions/managing-access-to-actions-from-githubcom/enabling-automatic-access-to-githubcom-actions-using-github-connect#automatic-retirement-of-namespaces-for-actions-accessed-on-githubcom)”。 
...
* 必须在企业上创建可以创建并写入目标组织中的存储库的{% data variables.product.pat_generic %}。 有关详细信息，请参阅“[创建{% data variables.product.pat_generic %}](/github/authenticating-to-github/creating-a-personal-access-token)”。{% ifversion ghes %}
* 如果想同步 {% data variables.location.product_location %}上 `actions` 组织中的捆绑操作，你需要是 `actions` 组织的所有者。
...
1. Run the `actions-sync sync` command:
   ...
   * `--destination-token`：目标企业实例的{% data variables.product.pat_generic %}。
