...
- `<Audience>` 元素必须始终作为 `<AudienceRestriction>` 元素的一部分提供。 此元素必须匹配 {{ site.data.variables.product.prodname_ghe_server }} 实体 ID。 这是 {{ site.data.variables.product.prodname_ghe_server }} 实例的 URL，如 `https://ghe.corp.example.com`。
...
```
如果 `Audience` 缺失或者与 {{ site.data.variables.product.prodname_ghe_server }} 实体 Id 不匹配，身份验证日志中将显示以下错误消息：
