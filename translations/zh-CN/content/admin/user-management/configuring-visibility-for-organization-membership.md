---
title: 配置组织成员关系的可见性
intro: 您可以将实例中新组织成员的可见性设置为公开或私密。 您还可以阻止成员将其可见性改为非默认值。
redirect_from:
...
4. 或者，为了阻止成员将他们的成员关系可见性改为非默认值，也可以选择 **Enforce on organization members**。 ![用于在所有成员中强制使用默认设置的复选框](/assets/images/enterprise/site-admin-settings/enforce-default-org-membership-visibility-setting.png)
5. 如果您想在所有现有成员中强制使用新的可见性设置，请使用 `ghe-org-membership-update` 命令行实用程序。 更多信息请参阅“[命令行实用程序](/enterprise/{{ currentVersion }}/admin/guides/installation/command-line-utilities#ghe-org-membership-update)”。
