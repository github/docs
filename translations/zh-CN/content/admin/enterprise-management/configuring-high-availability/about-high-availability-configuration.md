...
`ghe-repl-setup` 命令将 {% data variables.product.prodname_ghe_server %} 设备置于副本备用模式。
...
`ghe-repl-start` 命令启用所有数据存储的主动复制。
...
`ghe-repl-teardown` 命令完全禁用复制模式，并移除副本配置。
