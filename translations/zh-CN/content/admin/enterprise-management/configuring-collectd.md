...
{{ site.data.variables.product.prodname_enterprise }} ships with `collectd` version 5.x. `collectd` 5.x is not backwards compatible with the 4.x release series. 中央 `collectd` 服务器的版本至少需要是 5.x 才能接受从 {{ site.data.variables.product.product_location_enterprise }} 发送的数据。
