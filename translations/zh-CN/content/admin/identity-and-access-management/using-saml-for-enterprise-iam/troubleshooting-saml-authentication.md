...
intro: '如果使用 SAML 单一登录 (SSO)，并且用户无法通过身份验证来访问 {% data variables.location.product_location %}，可以排查问题。'
versions:
  ghes: '*'
  ghec: '*'
type: how_to
...
- 在将 {% data variables.location.product_location %} 的新身份验证设置应用到生产环境之前，请在过渡环境中测试这些设置。 有关详细信息，请参阅“[设置暂存实例](/admin/installation/setting-up-a-github-enterprise-server-instance/setting-up-a-staging-instance)”。
...
1. 尝试通过 SAML IdP 登录 {% data variables.location.product_location %}。

1. 在 {% data variables.location.product_location %} 上查看“/var/log/github/auth.log”中的调试输出。
...
auth.log 中的某些输出可能是 Base64 编码的。 可以访问管理 shell 并使用 {% data variables.location.product_location %} 上的 `base64` 实用工具来解码这些响应。 有关详细信息，请参阅“[访问管理 shell (SSH)](/admin/configuration/configuring-your-enterprise/accessing-the-administrative-shell-ssh)”。
...
当用户首次使用 SAML 身份验证登录 {% data variables.location.product_location %} 时，{% data variables.product.product_name %} 会在实例上创建一个用户帐户并将 SAML `NameID` 映射到该帐户。
...
如果 `Recipient` 与 {% data variables.location.product_location %} 的 ACS URL 不匹配，当用户尝试进行身份验证时，身份验证日志中将显示以下两条错误消息之一。
...
请确保将 IdP 上的 `Recipient` 的值设置为 {% data variables.location.product_location %} 的完整 ACS URL。 例如 `https://ghe.corp.example.com/saml/consume`。
...
请确保将 IdP 上的 `Audience` 的值设置为 {% data variables.location.product_location %} 的 `EntityId`，这是实例的完整 URL。 例如，`https://ghe.corp.example.com`。
{% endif %}
