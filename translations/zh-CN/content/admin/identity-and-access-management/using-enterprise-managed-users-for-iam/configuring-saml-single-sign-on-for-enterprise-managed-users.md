...
ms.locfileid: '148180051'
---
## 关于 {% data variables.product.prodname_emus %} 的 SAML 单一登录

通过 {% data variables.product.prodname_emus %}，你的企业使用你的企业标识提供者 (IdP) 对所有成员进行身份验证。 你的企业成员将通过 IdP 登录，而不是使用 {% data variables.product.prodname_dotcom %} 用户名和密码登录到 {% data variables.product.prodname_dotcom %}。
...
若要为 {% data variables.enterprise.prodname_emu_enterprise %} 配置 SAML SSO，必须在 IdP 上配置应用程序，然后在 GitHub.com 上配置企业。 配置 SAML SSO 后，可以配置用户预配。 
...
1. 单击“ **保存**”。
    ...
    注意：当企业需要 SAML SSO 时，安装用户将不再有权访问企业，但将保持登录到 GitHub。 只有 IdP 预配的 {% data variables.enterprise.prodname_managed_users %} 才能访问企业。
