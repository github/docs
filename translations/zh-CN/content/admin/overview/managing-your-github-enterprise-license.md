...
您可以将 {{ site.data.variables.product.prodname_enterprise }} 许可中包含的用户许可分配给 {{ site.data.variables.product.product_location_enterprise }} 和 {{ site.data.variables.product.prodname_ghe_cloud }} 企业帐户中的用户。 将用户添加到任一环境时，他们都会占用一个许可。 如果用户在两种环境中都有帐户，要想仅使用一个许可，其主 {{ site.data.variables.product.prodname_enterprise }} 电子邮件地址必须与经过验证的 {{ site.data.variables.product.prodname_ghe_cloud }} 电子邮件地址相同。 您可以在两种环境之间同步许可数和使用情况。
...
5. 作为站点管理员登录到您的 {{ site.data.variables.product.prodname_ghe_server }} 实例。
