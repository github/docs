...
| 包管理器           | YAML value       | Supported versions        | Private repositories |  供应   |
| -------------- | ---------------- | ------------------------- |:--------------------:|:-----:|
| Bundler        | `bundler`        | v1                        |                      | **✓** |
| Cargo          | `cargo`          | v1                        |        **✓**         |       |
...
For package managers such as `pipenv` and `poetry`, you need to use the `pip` YAML value. 例如，如果您使用 `poetry` 来管理 Python 依赖项，并且希望 {% data variables.product.prodname_dependabot %} 监控新版本的依赖项清单文件，请在 *dependabot.yml* 文件中使用 `package-ecosystem: "pip"`。
