1. 在 {% ifversion fpt or ghes > 3.1 or ghae-next %}“运行器”{% else %}“自托管运行器”{% endif %} 下，找到列表中的运行器。 如果您的运行器在一个组中，请单击 {% octicon "chevron-down" aria-label="The downwards chevron" %} 以展开列表。
