---
title: '{% data variables.product.prodname_projects_v2 %}를 사용하여 계획 및 추적'
shortTitle: '{% data variables.product.prodname_projects_v2 %}'
intro: '{% data variables.product.company_short %}에서 작업을 추적하는 조정 가능한 프로젝트를 빌드합니다.'
versions:
  feature: projects-v2
topics:
  - Issues
  - Projects
children:
  - /learning-about-projects
  - /creating-projects
  - /managing-items-in-your-project
  - /understanding-fields
  - /customizing-views-in-your-project
  - /automating-your-project
  - /viewing-insights-from-your-project
  - /managing-your-project
allowTitleToDifferFromFilename: true
redirect_from:
  - /issues/trying-out-the-new-projects-experience
ms.openlocfilehash: 89e7ed2a98f1b29f26d268622b639a893ab0c552
ms.sourcegitcommit: e8c012864f13f9146e53fcb0699e2928c949ffa8
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/09/2022
ms.locfileid: '148159239'
---

