...
For an example of how to use the Checks API with a {{ site.data.variables.product.prodname_github_app }}, see "[Creating CI tests with the Checks API](/apps/quickstart-guides/creating-ci-tests-with-the-checks-api/)."
...
If a check run is in a incomplete state for more than 14 days, then the check run's `conclusion` becomes `stale` and appears on {{ site.data.variables.product.prodname_dotcom }} as stale with
{% octicon "issue-reopened" aria-label="The issue-reopened icon" %}. Only {{ site.data.variables.product.prodname_dotcom }} can mark check runs as `stale`. For more information about possible conclusions of a check run, see the [`conclusion` parameter](/rest/reference/checks#create-a-check-run--parameters).
{% endif %}
...
When you set up a check run with requested actions (not to be confused with {{ site.data.variables.product.prodname_actions }}), you can display a button in the pull request view on {{ site.data.variables.product.prodname_dotcom }} that allows people to request your {{ site.data.variables.product.prodname_github_app }} to perform additional tasks.
