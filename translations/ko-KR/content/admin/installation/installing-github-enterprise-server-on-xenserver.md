...
intro: 'To install {{ site.data.variables.product.prodname_ghe_server }} on XenServer, you must deploy the {{ site.data.variables.product.prodname_ghe_server }} disk image to a XenServer host.'
redirect_from:
...
- You must install the XenServer Hypervisor on the machine that will run your {{ site.data.variables.product.prodname_ghe_server }} virtual machine (VM). We support versions 6.0 through 7.0.
...
4. Select {{ site.data.variables.product.prodname_dotcom }} On-premises, then click **XenServer (VHD)**.
...
1. In XenCenter, import the {{ site.data.variables.product.prodname_ghe_server }} image you downloaded. For instructions, see the XenCenter guide "[Import Disk Images](https://docs.citrix.com/en-us/xencenter/current-release/vms-importdiskimage.html)."
...
{{ site.data.reusables.enterprise_installation.create-attached-storage-volume }} For instructions, see the XenCenter guide "[Add Virtual Disks](https://docs.citrix.com/en-us/xencenter/current-release/vms-storage-addnewdisk.html)."
