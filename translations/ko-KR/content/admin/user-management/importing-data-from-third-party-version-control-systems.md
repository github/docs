...
intro: 'Using the git-import suite of tools, you can import from Subversion, Mercurial and Team Foundation Version Control to Git repositories on {{ site.data.variables.product.prodname_ghe_server }}.'
redirect_from:
...
4. Rewrite the authors and branches using the CSV file:
  ...
  ```
5. If you haven't yet, [create a new empty repository on {{ site.data.variables.product.prodname_ghe_server }}](/enterprise/{{ currentVersion }}/user/articles/creating-a-new-repository).
{{ site.data.reusables.command_line.switching_directories_procedural }}
7. Push the imported repository to {{ site.data.variables.product.prodname_ghe_server }}:
  ```shell
