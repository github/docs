...
intro: 'IdP(ID 공급자)를 통해 {% data variables.product.product_name %}의 팀 및 조직 멤버 자격을 관리할 수 있습니다. IdP 그룹을 {% data variables.enterprise.prodname_emu_enterprise %} 내의 팀과 연결합니다.'
...
ms.locfileid: '148160826'
---
## {% data variables.product.prodname_emus %}를 사용하는 팀 관리 정보

{% data variables.product.prodname_emus %}를 사용하면 {% data variables.product.prodname_dotcom %} 팀을 IdP 그룹에 연결하여 IdP를 통해 엔터프라이즈 내에서 팀 및 조직 멤버 자격을 관리할 수 있습니다. 엔터프라이즈 조직 중 하나의 팀을 IdP 그룹에 연결하면 IdP 그룹의 멤버 자격 변경 내용이 엔터프라이즈에 자동으로 반영되므로 수동 업데이트, 사용자 지정 스크립트 등의 필요성이 줄어듭니다. 

IdP 그룹 또는 새 팀 연결을 변경하면 {% data variables.enterprise.prodname_managed_user %}이(가) 아직 구성원이 아닌 조직의 팀에 합류하게 되면 {% data variables.enterprise.prodname_managed_user %}이(가) 자동으로 조직에 추가됩니다. 팀에서 그룹의 연결을 끊으면 팀 멤버 자격을 통해 조직의 멤버가 된 사용자는 다른 방법으로 조직의 멤버 자격을 할당받지 않은 경우 조직에서 제거됩니다.

{% note %}

**참고:** 조직 소유자는 계정이 SCIM을 통해 이미 프로비전된 경우 조직에 {% data variables.enterprise.prodname_managed_users %}을(를) 수동으로 추가할 수도 있습니다.
...
## IdP 그룹을 팀과 연결하기 위한 요구 사항

{% data variables.product.prodname_dotcom %}의 팀과 IdP 그룹을 연결하려면 먼저 IdP의 {% data variables.product.prodname_emu_idp_application %} 애플리케이션에 그룹을 할당해야 합니다. 자세한 내용은 "[{% data variables.product.prodname_emus %}에 대한 SCIM 프로비저닝](/admin/identity-and-access-management/using-enterprise-managed-users-for-iam/configuring-scim-provisioning-for-enterprise-managed-users) 정보"를 참조하세요.
