---
title: GitHub Codespaces에 대한 C# (.NET) 프로젝트 설정
shortTitle: Setting up your C# (.NET) project
allowTitleToDifferFromFilename: true
intro: '사용자 지정 개발 컨테이너를 만들어 {% data variables.product.prodname_github_codespaces %}에서 C# (.NET) 프로젝트를 시작합니다.'
redirect_from:
...
ms.locfileid: '148158615'
---
## 소개

이 가이드에서는 C# (.NET) 프로젝트 {% data reusables.codespaces.setting-up-project-intro %}를 설정하는 방법을 보여 줍니다.

### 필수 조건

- {% data variables.product.prodname_dotcom_the_website %}의 리포지토리에 기존 C#(.NET) 프로젝트가 있어야 합니다. 프로젝트가 없는 경우 다음 예제(https://github.com/2percentsilk/dotnet-quickstart )를 사용하여 이 자습서를 사용해 볼 수 있습니다.
...
codespace를 만들면 프로젝트가 전용인 원격 VM에 만들어집니다. 기본적으로 codespace의 컨테이너에는 .NET을 비롯한 많은 언어와 런타임이 있습니다. 또한 git, wget, rsync, openssh, nano와 같은 일반적인 도구 세트가 포함되어 있습니다.

{% data reusables.codespaces.customize-vcpus-and-ram %}

## 2단계: 템플릿에서 리포지토리에 개발 컨테이너 구성 추가

{% data variables.product.prodname_github_codespaces %}의 기본 개발 컨테이너 또는 "개발 컨테이너"에는 최신 .NET 버전 및 일반적인 도구가 미리 설치되어 있습니다. 그러나 프로젝트에 필요한 모든 도구와 스크립트를 포함하도록 자체 개발 컨테이너를 구성하는 것이 좋습니다. 이렇게 하면 리포지토리의 모든 {% data variables.product.prodname_github_codespaces %} 사용자에게 완전히 재현 가능한 환경이 보장됩니다.

{% data reusables.codespaces.setup-custom-devcontainer %}

{% data reusables.codespaces.command-palette-container %}
1. 이 예제에서는 **C#(.NET)** 을 클릭합니다. 추가 기능이 필요한 경우 C#(.NET) 전용 컨테이너 또는 C#(.NET) 및 MS SQL과 같은 도구 조합을 선택할 수 있습니다.
  ![목록에서 C# (.NET) 옵션을 선택합니다.](/assets/images/help/codespaces/add-dotnet-prebuilt-container.png)
1. 권장되는 .NET 버전을 클릭합니다.
  ![.NET 버전 선택](/assets/images/help/codespaces/add-dotnet-version.png)
1. 사용자 지정에 Node.js를 추가하는 기본 옵션을 수락합니다.
  ![Node.js 선택 항목 추가](/assets/images/help/codespaces/dotnet-options.png) {% data reusables.codespaces.rebuild-command %}

### 개발 컨테이너 분석

C#(.NET) 개발 컨테이너 템플릿을 추가하면 다음 파일이 있는 프로젝트 리포지토리의 루트에 `.devcontainer` 폴더가 추가됩니다.
...
- **build** - 빌드 속성입니다.
  ...
  - **args**
    - **variant**: 이 파일에는 사용하려는 .NET Core 버전인 빌드 인수가 하나만 포함되어 있습니다.
...
- **extensions** - 기본값으로 포함된 확장입니다.
  - **ms-dotnettools.csharp** - Microsoft C# 확장은 IntelliSense, 린팅, 디버깅, 코드 탐색, 코드 형식 지정, 리팩터링, 변수 탐색기, 테스트 탐색기 등과 같은 기능을 포함하여 C# 개발을 다양하게 지원합니다.
...
3. codespace 설정 프로세스의 일부로 종속성을 복원하려면 `postCreateCommand`의 주석 처리를 제거합니다.
...
5. “코드 맞춤법 검사기” 확장이 설치되었는지 확인하여 변경 내용이 성공적으로 적용되었는지 확인합니다.

    ![확장 목록](/assets/images/help/codespaces/dotnet-extensions.png)

## 4단계: 애플리케이션 실행

이전 섹션에서는 `postCreateCommand`를 사용하여 `dotnet restore` 명령을 통해 패키지 집합을 설치했습니다. 이제 종속성이 설치되었으므로 애플리케이션을 실행할 수 있습니다.

1. `F5`를 누르거나 터미널에 `dotnet watch run`을 입력하여 애플리케이션을 실행합니다.
...
이제 {% data variables.product.prodname_github_codespaces %}에서 C# (.NET) 프로젝트 개발을 시작할 준비가 되었습니다. 다음은 고급 시나리오를 위한 몇 가지 추가 리소스입니다.
