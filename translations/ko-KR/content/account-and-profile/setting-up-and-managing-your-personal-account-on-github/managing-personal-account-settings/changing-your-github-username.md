...
intro: '인스턴스에서 기본 제공 인증을 사용하는 경우 {% ifversion fpt 또는 ghec %}{% 데이터 variables.product.prodname_dotcom_the_website %}{% elsif ghes %}{% 데이터 variables.location.product_location %}에서 계정의 사용자 이름을 변경할 수 있습니다.'
redirect_from:
...
**참고**: {% 데이터 variables.enterprise.prodname_emu_enterprise %}의 멤버는 사용자 이름을 변경할 수 없습니다. 엔터프라이즈의 IdP 관리자가 {% data variables.product.product_name %}에 대한 사용자 이름을 제어합니다. 자세한 내용은 “[{% data variables.product.prodname_emus %} 정보](/admin/authentication/managing-your-enterprise-users-with-your-identity-provider/about-enterprise-managed-users)”를 참조하세요.

{% elsif ghes %}

**참고**: LDAP 자격 증명 또는 SSO(Single Sign-On)를 사용하여 {% 데이터 variables.location.product_location %}에 로그인하는 경우 로컬 관리자만 사용자 이름을 변경할 수 있습니다. {% 데이터 variables.product.product_name %}에 대한 인증 방법에 대한 자세한 내용은 "[{% 데이터 variables.location.product_location %}에 대한 사용자 인증"을](/admin/authentication/authenticating-users-for-your-github-enterprise-server-instance) 참조하세요.
...
{% 데이터 variables.enterprise.prodname_emu_enterprise %}의 멤버인 경우 사용자 이름을 변경할 수 없습니다. {% data reusables.enterprise-accounts.emu-more-info-account %}
...
사용자 이름을 변경한 후 `https://{% data variables.command_line.backticks %}/previoususername`과 같은 이전 프로필 페이지로 연결되는 링크는 404 오류를 반환합니다. {% 데이터 variables.location.product_location %}의 계정 링크를 다른 곳에서 업데이트하는 것이 좋습니다.{% ifversion fpt 또는 ghec %}(예: LinkedIn 또는 Twitter 프로필{% endif %}).
...
사용자 이름을 변경한 후 모든 공용 또는 비밀 요지의 URL도 변경되고 이러한 URL에 대한 이전 링크는 404 오류를 반환합니다. 이러한 gists에 대한 링크를 공유한 위치로 업데이트하는 것이 좋습니다.
