...
{% data variables.product.prodname_dotcom %}에서 사용할 수 있는 두 가지 프로세스가 있습니다.

- 표준 프로세스: 취약성 보고자는 리포지토리의 보안 정책에 있는 연락처 정보를 사용하여 리포지토리 유지 관리자와 연락합니다. 그런 다음, 리포지토리 유지 관리자는 필요한 경우 초안 리포지토리 권고를 만듭니다.  
- 프라이빗 취약성 보고: 취약성 보고자는 초안 리포지토리 권고를 제안하고 조사 결과에 대한 세부 정보를 제공하여 리포지토리 유지 관리자에게 직접 및 비공개로 취약성 세부 정보를 공개합니다.
...
{% endnote %}
 ...
 유지 관리자로서 코드의 취약성을 공개하려면 먼저 {% data variables.product.prodname_dotcom %}의 패키지 리포지토리에 보안 공지 초안을 만듭니다. {% data reusables.security-advisory.security-advisory-overview %} 자세한 내용은 "[리포지토리 보안 권고 정보"를 참조하세요](/code-security/repository-security-advisories/about-github-security-advisories-for-repositories).
...
{% data reusables.security-advisory.private-vulnerability-reporting-enable %}
 ...
 프라이빗 취약성 보고는 취약성 보고자가 {% data variables.product.prodname_dotcom %} 내에서 리포지토리 유지 관리자에게 보안 위험을 비공개로 공개하고 리포지토리 유지 관리자에게 문제를 즉시 알리는 방식으로 쉽게 공개할 수 있는 방법을 제공합니다. 보안 연구원 및 리포지토리 유지 관리자에 대한 자세한 내용은 각각 "[보안 취약성 비공개 보고](/code-security/security-advisories/guidance-on-reporting-and-writing/privately-reporting-a-security-vulnerability)" 및 "[비공개로 보고된 보안 취약성 관리](/code-security/security-advisories/guidance-on-reporting-and-writing/managing-privately-reported-security-vulnerabilities)"를 참조하세요.

{% note %}

**참고**: 취약성이 포함된 리포지토리에 프라이빗 취약성 보고를 사용하도록 설정하지 않은 경우 보안 연구원과 리포지토리 유지 관리자는 위의 "[표준 프로세스](#standard-process)" 섹션에 설명된 지침을 따라야 합니다.
