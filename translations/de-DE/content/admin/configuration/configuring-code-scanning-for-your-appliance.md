...
### Informationen zu {{ site.data.variables.product.prodname_code_scanning }}

With {{ site.data.variables.product.prodname_code_scanning }}, developers can quickly and automatically analyze the code in a {{ site.data.variables.product.prodname_dotcom }} repository to find security vulnerabilities and coding errors.
...
After you configure a self-hosted runner, users can enable {{ site.data.variables.product.prodname_code_scanning }} for individual repositories on {{ site.data.variables.product.product_location_enterprise }}. The default {{ site.data.variables.product.prodname_code_scanning }} workflow uses the `on.push` event to trigger a code scan on every push to any branch containing the workflow file.
