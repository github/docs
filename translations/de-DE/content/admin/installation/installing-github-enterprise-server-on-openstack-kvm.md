...
intro: 'Um {{ site.data.variables.product.prodname_ghe_server }} auf OpenStack KVM zu installieren, müssen Sie über OpenStack-Zugriff verfügen und das {{ site.data.variables.product.prodname_ghe_server }} QCOW2-Image herunterladen.'
redirect_from:
...
4. Wählen Sie „{{ site.data.variables.product.prodname_dotcom }} On-premises“ ({{ site.data.variables.product.prodname_dotcom }} (lokal)) aus, und klicken Sie anschließend auf **OpenStack KVM (QCOW2)**.
...
1. Laden Sie in OpenStack Horizon das von Ihnen heruntergeladene {{ site.data.variables.product.prodname_ghe_server }}-Image hoch. For instructions, see the "Upload an image" section of the OpenStack guide "[Upload and manage images](https://docs.openstack.org/horizon/latest/user/manage-images.html)."
