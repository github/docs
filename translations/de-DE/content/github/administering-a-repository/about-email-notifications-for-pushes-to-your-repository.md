...
- Den SHA1 des Commits, einschließlich einer Verknüpfung zum Diff in {{ site.data.variables.product.product_name }}
...
6. Wenn Du Deinen eigenen Server verwendest, kannst Du die Integrität von E-Mails über das Token **Secret** (Geheim) überprüfen. Dieses Token wird mit der E-Mail als `Approved`-Header (Genehmigt) gesendet. Wenn der `Approved`-Header (Genehmigt) mit dem von Dir gesendeten Token übereinstimmt, kannst Du darauf vertrauen, dass die E-Mail von {{ site.data.variables.product.product_name }} stammt. ![Textfeld für das E-Mail-Geheimnis](/assets/images/help/settings/email_services_token.png)
7. Wähle optional **Send from author** (Senden vom Autor), damit E-Mails mit der E-Mail-Adresse des Freigebenden zugestellt werden. Andernfalls werden E-Mails von {{ site.data.variables.notifications.no_reply_address }} gesendet. ![Kontrollkästchen für E-Mail-Autor](/assets/images/help/settings/email_services_author.png)
