...
Alle Verbindungen, auch die zu Remote-URLs, müssen als „git“-Benutzer hergestellt werden. Wenn Du versuchst, die Verbindung unter Deinem {{ site.data.variables.product.product_name }}-Benutzernamen herzustellen, tritt ein Fehler auf:

```shell
$ ssh -T <em>GITHUB-USERNAME</em>@{{ site.data.variables.command_line.codeblock }}
> Permission denied (publickey).
```
Wenn Deine Verbindung fehlgeschlagen ist und Du eine Remote-URL mit Deinem {{ site.data.variables.product.product_name }}-Benutzernamen verwendest, kannst Du [die Remote-URL für die Verwendung des „git“-Benutzers ändern](/articles/changing-a-remote-s-url/).
...
Der Befehl `ssh-add` *sollte* eine lange Zeichenfolge an Ziffern und Buchstaben ausgeben. Wenn der Befehl nichts ausgibt, musst Du [einen neuen SSH-Schlüssel erzeugen](/articles/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent) und ihn mit {{ site.data.variables.product.product_name }} verknüpfen.
...
Um zu prüfen, ob der Schlüssel verwendet wird, kannst Du auch versuchen, eine Verbindung zu `git@{{ site.data.variables.command_line.backticks }}` herzustellen:
...
Du musst Deinen öffentlichen Schlüssel für {{ site.data.variables.product.product_name }} bereitstellen, um eine sichere Verbindung herzustellen.
...
6. Vergleichen Sie die Liste der SSH-Schlüssel mit der Ausgabe des Befehls `ssh-add`. ![SSH-Schlüssel-Auflistung in {{ site.data.variables.product.product_name }}](/assets/images/help/settings/ssh_key_listing.png)
...
Wenn Du Deinen öffentlichen Schlüssel in {{ site.data.variables.product.product_name }} nicht siehst, musst Du [Deinen SSH-Schlüssel zu {{ site.data.variables.product.product_name }} hinzufügen](/articles/adding-a-new-ssh-key-to-your-github-account), um ihn mit Deinem Computer zu verknüpfen.

{% warning %}

**Warnung:** Wenn Du einen SSH-Schlüssel siehst, der Dir auf {{ site.data.variables.product.product_name }} unbekannt ist, solltest Du ihn sofort löschen und Dich für weitere Hilfe an den {{ site.data.variables.contact.contact_support }} wenden. Ein Dir unbekannter öffentlicher Schlüssel kann ein Hinweis auf ein Sicherheitsproblem sein. Weitere Informationen findest Du unter „[SSH-Schlüssel überprüfen](/articles/reviewing-your-ssh-keys).“
