...
intro: 'Du kannst Deine {{ site.data.variables.product.product_name }}-Identität zu Drittanbieter-Anwendungen verbinden, die OAuth verwenden. Wenn Du eine {{ site.data.variables.product.prodname_oauth_app }} autorisierst, solltest Du sicherstellen, dass die Anwendung vertrauenswürdig ist, und überprüfen, von wem sie entwickelt wurde und auf welche Daten sie zugreifen will.'
redirect_from:
...
Wenn eine {{ site.data.variables.product.prodname_oauth_app }} Dich anhand Deines {{ site.data.variables.product.product_name }}-Kontos identifizieren möchte, wird eine Seite mit den Kontaktdaten des Anwendungsentwicklers und einer Liste der Daten angezeigt, die angefordert werden.

{% if currentVersion == "free-pro-team@latest" %}

{% tip %}

**Tipp:** Du musst [Deine E-Mail-Adresse verifizieren](/articles/verifying-your-email-address), bevor Du eine {{ site.data.variables.product.prodname_oauth_app }} autorisieren kannst.

{% endtip %}

{% endif %}

### {{ site.data.variables.product.prodname_oauth_app }}-Zugriff

{{ site.data.variables.product.prodname_oauth_app }}s können *Lese*- oder *Schreib*zugriff auf Deine {{ site.data.variables.product.product_name }}-Daten haben.
...
{{ site.data.variables.product.prodname_oauth_app }}s können mehrere Arten von Daten anfordern.
...
| Benachrichtigungen        | Der Benachrichtigungszugriff erlaubt es Apps, Deine {{ site.data.variables.product.product_name }}-Benachrichtigungen wie Kommentare zu Issues und Pull Requests zu lesen. Die Apps können jedoch auf keine Elemente in Deinem Repositorys zugreifen.                                                                           |
...
Wenn {{ site.data.variables.product.prodname_oauth_app }}s neue Zugriffsrechte anfordern, informieren sie Dich über die Unterschiede zwischen den aktuellen und den neuen Berechtigungen.

{% if currentVersion == "free-pro-team@latest" %}

### {{ site.data.variables.product.prodname_oauth_app }}s und Organisationen

Wenn Du eine {{ site.data.variables.product.prodname_oauth_app }} für Dein persönliches Benutzerkonto autorisierst, siehst Du auch, wie sich die Autorisierung auf jede Organisation auswirkt, bei der Du Mitglied bist.

- **Für Organisationen *mit* eingeschränktem {{ site.data.variables.product.prodname_oauth_app }}-Zugriff kannst Du verlangen, dass die Organisationsadministratoren die Verwendung der Anwendung in der Organisation genehmigen.** Wenn die Organisation die Anwendung nicht genehmigt, kann die Anwendung nur auf die öffentlichen Ressourcen der Organisation zugreifen. Wenn Du ein Organisationsadministrator bist, kannst Du [die Anwendung selbst genehmigen](/articles/approving-oauth-apps-for-your-organization).

- **Für Organisationen *ohne* eingeschränkten {{ site.data.variables.product.prodname_oauth_app }}-Zugriff wird der Zugriff der Anwendung auf die Ressourcen dieser Organisation automatisch genehmigt.** Daher solltest Du genau überlegen, für welche {{ site.data.variables.product.prodname_oauth_app }}s Du den Zugriff auf die Ressourcen Deines persönlichen Kontos und Deiner Organisation gewährst.

Wenn Du einer Organisation angehörst, die SAML Single-Sign-on erzwingt, musst Du eine aktive SAML-Sitzung für jede Organisation haben, jedes mal wenn Du eine {{ site.data.variables.product.prodname_oauth_app }} autorisierst.
