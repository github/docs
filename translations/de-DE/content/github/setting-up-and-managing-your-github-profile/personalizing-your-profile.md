...
intro: 'Du kannst Informationen zu Deiner Person für andere {{ site.data.variables.product.product_name }}-Benutzer bereitstellen, indem Du ein Profilbild einrichtest und eine Biografie zum Profil hinzufügst.'
redirect_from:
...
Mit Deinem Profilbild kannst Du überall auf {{ site.data.variables.product.product_name }} in Pull Requests, Kommentaren, Beiträge-Seiten und Diagrammen leichter identifiziert werden.

Wenn Du ein Konto anlegst, stellt {{ site.data.variables.product.product_name }} Dir ein zufällig generiertes „Identicon“ bereit. [Dein Identicon](https://github.com/blog/1586-identicons) wird aus einem Hash Deiner Benutzer-ID erzeugt. Seine Farbe und sein Muster lassen sich daher nicht steuern. Du kannst das Identicon durch ein Bild ersetzen, das Dich repräsentiert.
...
Sie können einen Status festlegen, um Informationen zu Ihrer aktuellen Verfügbarkeit auf {{ site.data.variables.product.product_name }} anzuzeigen. An folgenden Stellen wird Ihr Status angezeigt:
- auf Deiner {{ site.data.variables.product.product_name }}-Profilseite.
- wenn Benutzer auf {{ site.data.variables.product.product_name }} mit dem Mauszeiger über Deinen Benutzernamen oder Deinen Avatar fahren.
...
Wenn Sie Ihren Status festlegen, können Sie andere Benutzer auch darüber informieren, dass Sie auf {{ site.data.variables.product.product_name }} nur begrenzt verfügbar sind.
...
1. Klicke in der oberen rechten Ecke von {{ site.data.variables.product.product_name }} auf Dein Profilbild. Klicke anschließend auf **Set your status** (Deinen Status festlegen) oder, wenn Du bereits einen Status eingerichtet hast, auf den aktuellen Status. ![Schaltfläche im Profil zum Festlegen des Status](/assets/images/help/profile/set-status-on-profile.png)
...
| {% octicon "star-fill" aria-label="The star icon" %}            | **Pro**                                                                   | If you use {{ site.data.variables.product.prodname_pro }} you'll get a PRO badge on your profile. Weitere Informationen zu {{ site.data.variables.product.prodname_pro }} finden Sie unter „[Produkte von {{ site.data.variables.product.prodname_dotcom }}](/github/getting-started-with-github/githubs-products#github-pro)“.                                    |
