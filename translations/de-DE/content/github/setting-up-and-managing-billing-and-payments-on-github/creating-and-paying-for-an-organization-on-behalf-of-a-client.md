...
intro: 'Du kannst im Namen eines Kunden eine {{ site.data.variables.product.prodname_dotcom }}-Organisation erstellen und bezahlen.'
redirect_from:
...
- Der {{ site.data.variables.product.prodname_dotcom }}-Benutzername des Kunden, der Inhaber der Organisation werden soll, die Du erstellst
...
### Schritt 1: Erstelle Dein persönliches {{ site.data.variables.product.prodname_dotcom }}-Konto
...
Wenn Du bereits ein persönliches {{ site.data.variables.product.prodname_dotcom }}-Benutzerkonto hast, fahre gleich mit [Schritt 2](#step-2-create-the-organization) fort.
...
3. Wähle {{ site.data.variables.product.prodname_free_user }} für Dein persönliches Konto aus.
...
3. Klicke unter „Choose a plan“ (Plan auswählen) auf **Choose {{ site.data.variables.product.prodname_free_team }}** (auswählen). Im nächsten Schritt wirst Du die Organisation heraufstufen.
...
{{ site.data.reusables.dotcom_billing.choose_org_plan }} (Im nächsten Schritt kannst Du weitere Benutzer in die Organisation aufnehmen.)
...
5. Gib den {{ site.data.variables.product.prodname_dotcom }}-Benutzernamen Deines Kunden ein, und drücke die **Eingabetaste**. ![Feld zum Eingeben des Benutzernamens des Kunden](/assets/images/help/organizations/org-invite-modal.png)
...
2. Wenn Du die Kreditkarte Deiner Organisation aus der neuen Organisation entfernen möchtest, damit sie nicht mehr belastet wird, wende Dich an {{ site.data.variables.contact.contact_support }}.
