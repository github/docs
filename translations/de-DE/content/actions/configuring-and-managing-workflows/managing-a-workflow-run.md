...
Auf der Workflow-Lauf-Seite können sie sehen, ob ein Workflow-Lauf ausgeführt wird oder abgeschlossen ist. Wenn der Lauf ausgeführt wird, können Sie den Lauf abbrechen. Sie müssen mit einem {{ site.data.variables.product.prodname_dotcom }}-Konto angemeldet sein, um Workflow-Informationen anzuzeigen, auch für öffentliche Repositories. Weitere Informationen finden Sie unter „[Zugriffsberechtigungen auf GitHub](/articles/access-permissions-on-github)“.
...
{{ site.data.variables.product.prodname_actions }} verwenden die Checks API, um Status, Ergebnisse und Protokolle für einen Workflow auszugeben. {{ site.data.variables.product.prodname_dotcom }} erstellt eine neue Prüfsuite für jeden Workflow-Lauf. Die Prüfsuite enthält einen Prüflauf für jeden Auftrag im Workflow, und jeder Auftrag enthält Schritte. {{ site.data.variables.product.prodname_actions }} werden als Schritt in einem Workflow ausgeführt. Weitere Informationen zur Prüf-API finden Sie unter "[](/v3/checks/)".

{{ site.data.reusables.github-actions.invalid-workflow-files }}

### Ihren Workflow-Verlauf anzeigen

Sie können jeden Auftrag in einem Workflow-Lauf und jeden Schritt in einem Auftrag anzeigen. Weitere Informationen findest Du unter „[Kernkonzepte für {{ site.data.variables.product.prodname_actions }}](/actions/automating-your-workflow-with-github-actions/core-concepts-for-github-actions#job)“.  {{ site.data.reusables.repositories.permissions-statement-read }}
...
Bei Aufträgen, die auf {{ site.data.variables.product.prodname_dotcom }}-gehosteten Läufern ausgeführt werden, zeichnet "Job einrichten" Details der virtuellen Umgebung des Läufers auf und enthält einen Link zur Liste der vorinstallierten Tools, die auf dem Läufercomputer vorhanden waren.
...
Wenn Sie einen Workflowlauf abbrechen, bricht {{ site.data.variables.product.prodname_dotcom }} alle Aufträge und Schritte ab, die Teil dieses Workflows sind. {{ site.data.reusables.repositories.permissions-statement-write }}

Beim Abbrechen der Workflowausführung führen Sie möglicherweise andere Software aus, die Ressourcen verwendet, die mit der Workflowausführung zusammenhängen. Um Ihnen zu helfen, Ressourcen im Zusammenhang mit der Workflowausführung freizugeben, kann es hilfreich sein, die Schritte zu verstehen, {{ site.data.variables.product.prodname_dotcom }} zum Abbrechen einer Workflowausführung ausführt. Weitere Informationen finden Sie unter "[Schritte, die {{ site.data.variables.product.prodname_dotcom }} zum Abbrechen eines Workflowlaufs](#steps-github-takes-to-cancel-a-workflow-run).
...
#### Schritte {{ site.data.variables.product.prodname_dotcom }} zum Abbrechen einer Workflowausführung führt
...
Sie können eine Workflowausführung löschen, die abgeschlossen wurde oder mehr als 2 Wochen alt ist. {{ site.data.reusables.repositories.permissions-statement-write }}
...
Fakturierbare Auftragsausführungsminuten werden nur für Aufträge angezeigt, die auf privaten Repositorys ausgeführt werden, die {{ site.data.variables.product.prodname_dotcom }}gehosteten Läufer verwenden. Es gibt keine fakturierbaren Minuten, wenn {{ site.data.variables.product.prodname_actions }} in öffentlichen Repositorys oder für Aufträge verwendet werden, die auf selbst gehosteten Läufern ausgeführt werden.
...
1. Klicken Sie in der Auftragszusammenfassung auf **Ausführen und fakturierbare Zeitdetails**. ![Link zu Den Ausführungs- und Abrechnungsdetails](/assets/images/help/repository/view-run-billable-time.png)
   ...
   **Hinweis:** Die angezeigte Abrechnungszeit enthält keine Rundungen oder Minutenmultiplikatoren. Informationen zum Anzeigen der gesamten {{ site.data.variables.product.prodname_actions }} Nutzung, einschließlich Rundungs- und Minutenmultiplikatoren, finden Sie unter "[Anzeigen Ihrer {{ site.data.variables.product.prodname_actions }} Nutzung](/github/setting-up-and-managing-billing-and-payments-on-github/viewing-your-github-actions-usage)".
...
Sie können die Build-Protokolle für einen bestimmten Schritt durchsuchen. Beim Durchsuchen von Protokollen werden nur eingeblendete Schritte in die Ergebnisse einbezogen. {{ site.data.reusables.repositories.permissions-statement-read }}
...
Du kannst die Logdateien aus Deiner Workflow-Ausführung löschen. {{ site.data.reusables.repositories.permissions-statement-write }}
...
- „[Informationen zu {{ site.data.variables.product.prodname_actions }}](/articles/about-github-actions)“
- „[Einen Workflow konfigurieren](/articles/configuring-a-workflow)“
- „[Workflow-Syntax für {{ site.data.variables.product.prodname_actions }}](/articles/workflow-syntax-for-github-actions)“
