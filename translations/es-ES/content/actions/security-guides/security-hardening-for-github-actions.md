...
1. **El `GITHUB_TOKEN`**
    -  A este token se le da un alcance intencional para el repositorio sencillo que invocó el flujo de trabajo y puede tener el mismo nivel de acceso que el de un usuario con acceso de escritura a dicho repositorio. El token se crea antes de que inicie cada job y caduca cuando dicho job finaliza. Para obtener más información, consulta "[Autenticar con el GITHUB_TOKEN](/actions/configuring-and-managing-workflows/authenticating-with-the-github_token)".
