...
ms.locfileid: "145069766"
---
Autor: Albert Ziegler (@wunderalbert)

## <a name="-data-variablesproductprodname_dotcom--copilot-parrot-or-crow"></a>Copiloto de {% data variables.product.prodname_dotcom %}: ¿Perico o Cuervo?
...
¿Qué tan directa es la relación entre el código que se sugiere y el que lo informó? En un artículo reciente que invita a la reflexión<sup id="anchor1">[1](#footnote1)</sup>, Bender, Gebru et al. acuñaron la frase "papagayos estocásticos" para los sistemas de inteligencia artificial como en los que se basa {% data variables.product.prodname_dotcom %} Copilot. O, como comentó un ingeniero de aprendizaje automático en {% data variables.product.company_short %}<sup id="anchor2">[2](#footnote2)</sup> durante una charla junto a un dispensador de agua: estos sistemas pueden percibirse como "un niño con memoria fotográfica".

Estas son simplificaciones excesivas y deliberadas. Muchas de las sugerencias del Copiloto de {% data variables.product.prodname_dotcom %} se sienten como si se hubieran hecho a la talla para el código base específico en el que está trabajando el usuario. A menudo, parecen más un cuervo construyendo herramientas nuevas a partir de pequeños bloques<sup id="anchor3">[3](#footnote3)</sup> que un papagayo. Pero no se puede negar que el Copiloto de {% data variables.product.prodname_dotcom %} tiene una memoria impresionante:

![Una demostración animada del Copiloto](/assets/images/help/copilot/resources_recitation_example_zen.gif)

Aquí, he ordenado intencionadamente<sup id="anchor4">[4](#footnote4)</sup> a {% data variables.product.prodname_dotcom %} Copilot para recitar un texto conocido que obviamente se sabe de memoria. Yo también me sé un par de textos de memoria. Por ejemplo, aún recuerdo algunos poemas que aprendí en la escuela. Sin embargo, sin importar el tema, jamás me he atrevido a entorpecer una conversación hablando en tetrámetro yámbico y con emoción sobre los narcisos.
...
## <a name="the-experiment"></a>El Experimento
...
### <a name="automatic-filtering"></a>Filtrado Automático
...
![Ejemplo de código](/assets/images/help/copilot/example_last_straw.png)
...
### <a name="manual-bucketing"></a>Agrupamiento Manual en Buckets
...
2. Algunas secuencias fueron largas y repetitivas. Al igual que en el ejemplo siguiente, donde los bloques repetidos de `‘<p>’` se encuentran por supuesto en algún lugar del conjunto de entrenamiento: <br>![Repeticiones de ejemplo](/assets/images/help/copilot/example_repetitions.png)<br> Estas sugerencias pueden ser útiles (casos de prueba, expresiones regulares) o inútiles (como en este caso, sospecho). Pero, de cualquier forma, no encajan en la idea de aprender por repetición que tenía cuando comencé esta investigación.
...
Necesariamente, este depósito tiene algunos casos perimetrales<sup id="anchor6">[6](#footnote6)</sup>, y su kilometraje puede variar en función de cómo cree que deban clasificarse. Posiblemente incluso estés en desacuerdo con todo el conjunto de buckets en primera instancia.

Por eso hemos lanzado ese conjunto de datos<sup id="anchor7">[7](#footnote7)</sup> en código abierto. Así que, si piensan diferente sobre el agrupamiento en buckets o si les interesa otros aspectos de cómo el Copiloto de GitHub repite su conjunto de entrenamiento, los invito a que ignoren mi siguiente sección y de que saquen sus propias conclusiones.
...
Esto corresponde a **1 evento de recitación cada 10 semanas de usuario** (intervalo de confianza del 95 %: 7-13 semanas, mediante una prueba de Poisson).
...
### <a name="-data-variablesproductprodname_dotcom--copilot-quotes-when-it-lacks-specific-context"></a>Citas del Copiloto de {% data variables.product.prodname_dotcom %} cuando le falta contexto específico
...
### <a name="-data-variablesproductprodname_dotcom--copilot-mostly-quotes-in-generic-contexts"></a>El Copiloto de {% data variables.product.prodname_dotcom %} cita mayormente en contextos genéricos

Conforme pasa el tiempo, cada archivo se convierte en único. Pero {% data variables.product.prodname_dotcom %} Copilot no espera<sup id="anchor8">[8](#footnote8)</sup>: ofrecerá soluciones aunque el archivo siga siendo extremadamente genérico. Y, en la ausencia de cualquier cosa que sea específica para seguir, es mucho más probable citar de cualquier otro lugar que de esta.
...
![Ejemplo de código](/assets/images/help/copilot/example_robot.png)

Esto se extrae casi directamente de los cursos de una clase de robótica cargada en diferentes variaciones<sup id="anchor9">[9](#footnote9)</sup>.

### <a name="detection-is-only-as-good-as-the-tool-that-does-the-detecting"></a>La detección dependerá de la herramienta que realiza la detección
...
## <a name="conclusion-and-next-steps"></a>Conclusión y siguientes pasos

Esta investigación demuestra que {% data variables.product.prodname_dotcom %} Copilot _puede_ citar un cuerpo de código literal, pero es raro que lo haga y, cuando lo hace, cita en su mayoría el código que todos citan y, mayormente, al inicio de un archivo, como para romper el hielo.

Pero aún existe una gran diferencia entre que GitHub Copilot recite el código y que yo recite un poema: Yo _sé_ que lo estoy citando. También me gustaría saber cuando el Copiloto está repitiendo el código existente en vez de generar sus propias ideas. De esta forma, puedo ver la información de fondo sobre el código e incluir el crédito donde deba hacerse.
...
<a name="footnote3">3</a>: Véase von Bayern et al. acerca de la sabiduría creativa de los cuervos: [Compound tool construction by New Caledonian crows](https://www.nature.com/articles/s41598-018-33458-z) [^](#anchor3)

<a name="footnote4">4</a>: Véase Carlini et al. sobre la activación deliberada de la recuperación de datos de entrenamiento: [Extracting Training Data from Large Language Models](https://arxiv.org/pdf/2012.07805.pdf) [^](#anchor4)
...
<a name="footnote6">6</a>: Aunque puede que no _demasiados_. Les pedí a varios desarrolladores que me ayudaran a etiquetar los casos y se les pidió a todos marcar cualquier incerteza sobre su juicio. Esto sucedió solo en 34 casos, es decir, en menos del 10%. [^](#anchor6)

<a name="footnote7">7</a>: En el [conjunto de datos público](/assets/images/help/copilot/matched_snippets.csv), enumero la parte de la sugerencia de Copilot que también se encontró en el conjunto de entrenamiento, la frecuencia con la que se encontró y un vínculo a un ejemplo en el que se produce en el código público. Por razones de privacidad, no incluyo la parte sin empatar de la compleción o del contexto del código que hubiese escrito el usuario (solo una indicación de su longitud). [^](#anchor7)

<a name="footnote8">8</a>: De hecho, como se realizó este experimento, {% data variables.product.prodname_dotcom %} Copilot _ha_ cambiado para requerir un contenido mínimo de archivo. Así que, algunas de las sugerencias que se marcaron aquí no se mostraron en la versión actual. [^](#anchor8)
