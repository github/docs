...
* Debes crear un token de acceso personal (PAT) en tu empresa que pueda crear y escribir en los repositorios de las organizaciones destino. Para obtener más información, vea "[Crear un token de acceso personal](/github/authenticating-to-github/creating-a-personal-access-token)".{% ifversion ghes %}
* Si quiere sincronizar las acciones incluidas en la organización `actions` en {% data variables.product.product_location %}, debe ser un propietario de la organización `actions`.
...
1. Run the `actions-sync sync` command:
   ...
   * `--destination-token`: un token de acceso personal para la instancia de empresa de destino.
