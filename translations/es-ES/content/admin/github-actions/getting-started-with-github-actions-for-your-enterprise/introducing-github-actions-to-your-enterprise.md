...
{% ifversion actions-caching %}{% data variables.product.prodname_actions %} también tiene un sistema de almacenamiento en caché que puedes utilizar para guardar las dependencias en caché y acelerar las ejecuciones de flujo de trabajo. Para obtener más información, consulta la sección "[Almacenar las dependencias en caché para agilizar los flujos de trabajo](/actions/using-workflows/caching-dependencies-to-speed-up-workflows)".{% endif %}
