{{ site.data.variables.product.prodname_actions }}の利用にはいくつかの制限があり、それらは使用しているのが{{ site.data.variables.product.prodname_dotcom }}ホストランナーなのか、セルフホストランナーなのかによって変わります。 これらの制限は変更されることがあります。
