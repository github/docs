...
フォークされたリポジトリからベースリポジトリに対するプルリクエストを作成した場合、{{ site.data.variables.product.prodname_dotcom }}はベースリポジトリに対して`pull_request`イベントを送り、フォークされたリポジトリではプルリクエストイベントは生じません。
