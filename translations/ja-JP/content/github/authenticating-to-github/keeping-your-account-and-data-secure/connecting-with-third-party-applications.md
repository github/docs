...
| Gist                  | [Gist](https://gist.github.com) アクセスがあれば、アプリケーションは {% if currentVersion != "github-ae@latest" %}あなたのパブリックおよび{% else %}内部{% endif %}シークレット Gist の両方を読み書きできます。                            |
...
| リポジトリ                 | リポジトリ情報には、コントリビュータの名前、あなたが作成したブランチ、リポジトリ内の実際のファイルなどが含まれます。 アプリケーションは、ユーザ単位で{% if currentVersion != "github-ae@latest" %}パブリック{% else %}内部{% endif %}またはプライベートリポジトリへのアクセスをリクエストできます。     |
