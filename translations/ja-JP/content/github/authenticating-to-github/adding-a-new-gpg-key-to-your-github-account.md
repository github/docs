...
intro: '{{ site.data.variables.product.product_name }} アカウントが新しい (あるいは既存の) GPG キーを使うように設定するには、そのキーを {{ site.data.variables.product.product_name }} アカウントに追加する必要もあります。'
redirect_from:
...
新しい GPG キーを {{ site.data.variables.product.product_name }} アカウントに追加する前に、以下のことを済ませておかなければなりません:
...
署名の検証の際には、署名が抽出され、そのキー id のパースが試行されます。 そのキー id が、{{ site.data.variables.product.product_name }}にアップロードされたキーと照合されます。 GPG キーが {{ site.data.variables.product.product_name }}にアップロードされるまで、署名は検証できません。
...
6. 処理を確認するには、{{ site.data.variables.product.product_name }}のパスワードを入力します。
