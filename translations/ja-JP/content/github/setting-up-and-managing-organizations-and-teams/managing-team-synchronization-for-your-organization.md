...
intro: '{{ site.data.variables.product.product_name }} 上のアイデンティティプロバイダ (IdP) と Organization の間で Team の同期の有効/無効を切り替えることができます。'
...
permissions: Organization のオーナーは、Organization の Team 同期を管理できます。
...
IdP と {{ site.data.variables.product.product_name }} の間で Team の同期を有効化すると、Organization のオーナーとチームメンテナが Organization の Team を IdP グループに接続できるようになります。
