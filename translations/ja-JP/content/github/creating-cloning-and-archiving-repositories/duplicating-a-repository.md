...
リポジトリを複製してリポジトリの新しいコピー、つまり_ミラー_にプッシュできるようにするには、その前に {{ site.data.variables.product.product_location }} に[新しいリポジトリを作成](/articles/creating-a-new-repository)する必要があります。 以下の例では、`exampleuser/new-repository` および `exampleuser/mirrored` がミラーです。
...
### {{ site.data.variables.large_files.product_name_long }} オブジェクトを含むリポジトリをミラーする
...
3. クローンしたリポジトリに移動します。
  ...
  $ cd <em>old-repository</em>.git
  ```
4. リポジトリの {{ site.data.variables.large_files.product_name_long }} オブジェクトをプルします。
  ```shell
  $ git lfs fetch --all
  ```
5. 新しいリポジトリをミラープッシュします。
  ...
  $ git push --mirror https://{{ site.data.variables.command_line.codeblock }}/<em>exampleuser</em>/<em>new-repository</em>.git
  ```
6. リポジトリの {{ site.data.variables.large_files.product_name_long }} オブジェクトをミラーにプッシュします。
  ```shell
