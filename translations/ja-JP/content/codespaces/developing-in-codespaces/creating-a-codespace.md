...
{% data variables.product.prodname_codespaces %} を Organization 内で使用するには、所有者または支払いマネージャーが使用制限を設定しておく必要があります。 詳細については、「[Codespaces の利用制限の管理](/billing/managing-billing-for-github-codespaces/managing-spending-limits-for-codespaces#about-spending-limits-for-codespaces)」を参照してください。
...
リポジトリ、ブランチ、開発コンテナー構成ファイル (複数ある場合)、マシンの種類 (複数ある場合) を選ぶように求められます。
