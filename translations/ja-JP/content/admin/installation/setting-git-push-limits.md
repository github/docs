...
**注：**{{ site.data.variables.large_files.warning_size }}以上のサイズのファイルのみが、Gitプッシュの制限に照らし合わせて確認されます。 プッシュ制限をより小さくする必要がある場合は、{{ site.data.variables.contact.contact_ent_support }}までお問い合わせください。
...
5. あるいは、{{ site.data.variables.product.product_location_enterprise }}上のすべてのリポジトリにアップロードの最大制限を適用するために、**Enforce on all repositories（すべてのリポジトリに適用）**を選択してください。 ![すべてのリポジトリにオブジェクトの最大サイズを適用するオプション](/assets/images/enterprise/site-admin-settings/all-repo-upload-limit-option.png)
