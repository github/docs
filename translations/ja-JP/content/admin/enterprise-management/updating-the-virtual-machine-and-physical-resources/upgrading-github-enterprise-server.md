...
{% endif %}
7. 単一アプライアンスのアップグレードの場合は、メンテナンス モードを無効にして、ユーザーが {% data variables.product.product_location %} を使用できるようにします。
...
{% data reusables.enterprise_installation.replication-status %}コマンドから `Replication is not running` が返される場合、レプリケーションはまだ起動中の可能性があります。 約 1 分待ってから、もう一度 `ghe-repl-status` を実行してください。
   ...
   `ghe-repl-status` から `OK` が返されなかった場合は、{% data variables.contact.enterprise_support %} にお問い合わせください。 詳細については、「[{% data variables.contact.github_support %} からのヘルプの受信](/admin/enterprise-support/receiving-help-from-github-support)」を参照してください。
   
6. 最後のレプリカのアップグレードが完了し、再同期が完了したら、メンテナンス モードを無効にして、ユーザーが {% data variables.product.product_location %} を使用できるようにします。
