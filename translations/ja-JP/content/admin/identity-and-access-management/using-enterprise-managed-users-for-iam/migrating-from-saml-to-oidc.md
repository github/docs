...
intro: 'SAML を使用して {% data variables.product.prodname_emu_enterprise %} のメンバーを認証している場合は、OpenID Connect (OIDC) に移行すると、IdP の条件付きアクセス ポリシーのサポートからメリットを得ることができます。'
...
ms.locfileid: '147684452'
---
{% data reusables.enterprise-accounts.oidc-beta-notice %}

## {% data variables.product.prodname_emu_enterprise %} の SAML から OIDC への移行について

{% data variables.product.prodname_emu_enterprise %} が SAML SSO を使用して Azure Active Directory (Azure AD) で認証を行う場合は、OIDC に移行できます。 {% data reusables.enterprise-accounts.emu-cap-validates %}

SAML から OIDC に移行する場合、以前に SAML 用にプロビジョニングされたが、{% data variables.product.prodname_emu_idp_oidc_application %} アプリケーションによってプロビジョニングされていない{% data variables.product.prodname_managed_users %}とグループには、表示名に "(SAML)" が付加されます。
...
1. 新しいタブまたはウィンドウで、{% data variables.product.prodname_dotcom_the_website %} のセットアップ ユーザーとしてサインインしたまま、**admin:enterprise** スコープを持つ **有効期限のない** 個人用アクセス トークンを作成し、クリップボードにコピーします。 新しいトークンの作成について詳しくは、「[個人用アクセス トークンの作成](/github/setting-up-and-managing-your-enterprise/managing-your-enterprise-users-with-your-identity-provider/configuring-scim-provisioning-for-enterprise-managed-users#creating-a-personal-access-token)」を参照してください。
...
1. [シークレット トークン] に、先ほど作成した **admin:enterprise** スコープの個人用アクセス トークンを貼り付けます。
