...
| `--github-url`                         | ✓  | リポジトリがホストされる {% data variables.product.prodname_dotcom %} のインスタンス。 |{% if currentVersion ver_lt "enterprise-server@3.1" %}
...
| `--queries`                            |    | デフォルトのセキュリティクエリに加えて実行する、追加クエリのカンマ区切りリスト。 This overrides the `queries` setting in the custom configuration file.                                    |
