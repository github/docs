...
ms.locfileid: '146180208'
---
{% data reusables.actions.enterprise-beta %} {% data reusables.actions.enterprise-github-hosted-runners %}

## セルフホステッド ランナーの概要

セルフホスト ランナーとは、{% ifversion ghae or ghec %}{% data variables.product.product_name %}{% else %}{% data variables.product.product_location %}{% endif %} の {% data variables.product.prodname_actions %} からジョブを実行するためにデプロイおよび管理するシステムです。 {% data variables.product.prodname_actions %} の詳細については、「[{% data variables.product.prodname_actions %} について](/actions/learn-github-actions/understanding-github-actions)」{% ifversion fpt %}{% elsif ghec or ghes or ghae %}と「[エンタープライズの {% data variables.product.prodname_actions %} について](/admin/github-actions/getting-started-with-github-actions-for-your-enterprise/about-github-actions-for-enterprises)」{% endif %}を参照してください。
...
{% ifversion fpt or ghec %} セルフホスト ランナーでは {% data variables.product.product_location %} への接続を開くので、セルフホスト ランナーに対する受信接続を {% data variables.product.prodname_dotcom %} に許可する必要はありません。
{% elsif ghes or ghae %}ランナーから {% data variables.product.product_location %} への送信接続のみが必要となります。 {% data variables.product.product_location %} からランナーへの受信接続は必要ありません。
{%- endif %}

{% ifversion ghes %}

{% data variables.product.product_name %} では、{% data variables.product.product_location %} のホスト名と API サブドメインで {% ifversion ghes %}HTTP(S){% else %}HTTPS{% endif %} 経由のランナーからの受信接続を受け入れる必要があります。ランナーでは、{% ifversion ghes %}HTTP(S){% else %}HTTPS{% endif %} 経由の {% data variables.product.product_location %} のホスト名と API サブドメインへの送信接続を許可する必要があります。
...
さらに、ワークフローで他のネットワーク リソースへのアクセスが必要になる場合があります。 たとえば、ワークフローでパッケージをインストールしたり、コンテナーを {% data variables.product.prodname_dotcom %} パッケージに発行したりする場合、ランナーではそれらのネットワーク エンドポイントへのアクセスも必要になります。
...
{% ifversion ghes %}セルフホスト ランナーは、外部のインターネット アクセスを必要とせずに機能します。 その結果、ネットワーク ルーティングを使用して、セルフホスト ランナーと {% data variables.product.product_location %} の間の通信を管理できます。 たとえば、プライベート IP アドレスをセルフホスト ランナーに割り当て、トラフィックを {% data variables.product.product_location %} に送信するようにルーティングを構成できます。トラフィックが公衆ネットワークを通過する必要はありません。{% endif %}
...
{% data variables.product.product_location %} の {% data variables.product.prodname_dotcom_the_website %} アクションへの自動アクセスを有効にしている場合を除き、セルフホスト ランナーで {% data variables.product.prodname_dotcom_the_website %} に接続する必要はありません。 詳細については、「[エンタープライズでのアクションの使用について](/admin/github-actions/managing-access-to-actions-from-githubcom/about-using-actions-in-your-enterprise)」を参照してください。
