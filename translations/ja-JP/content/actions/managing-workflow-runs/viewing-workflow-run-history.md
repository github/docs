...
To specify the maximum number of runs to return, you can use the `-L` or `--limit` flag . 省略値は、`10` です。
...
Use the `--exit-status` flag to exit with a non-zero status if the run failed. 例:
