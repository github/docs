...
The `svc.sh` script uses `launchctl` to check whether the application is running. Например:
...
You can view the update activities in the *Runner_* log files. Например:
...
Check that the self-hosted runner's service account has permission to use the Docker service. You can identify this account by checking the configuration of the self-hosted runner in systemd. Например:
