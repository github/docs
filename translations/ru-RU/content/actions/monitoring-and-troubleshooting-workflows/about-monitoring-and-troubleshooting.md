...
Every workflow run generates a real-time graph that illustrates the run progress. You can use this graph to monitor and debug workflows. Например:
...
To identify how long a job took to run, you can view its execution time. Например:
...
You can view the status of each job and step in a workflow. Например:
...
Each workflow run generates activity logs that you can view, search, and download. Например:
