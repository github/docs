...
intro: 'A {{ site.data.variables.product.prodname_ghe_server }} cluster must be set up with a license and initialized using the administrative shell (SSH).'
redirect_from:
...
### Installing {{ site.data.variables.product.prodname_ghe_server }}

1. On each cluster node, provision and install {{ site.data.variables.product.prodname_ghe_server }}. For more information, see "[Setting up a {{ site.data.variables.product.prodname_ghe_server }} instance](/enterprise/{{ currentVersion }}/admin/guides/installation/setting-up-a-github-enterprise-server-instance)."
...
This example `cluster.conf` defines a cluster with five nodes.
  ...
  - Three nodes (called `ghe-data-node-\*`) run the services responsible for storage and retrieval of {{ site.data.variables.product.prodname_ghe_server }} data.
