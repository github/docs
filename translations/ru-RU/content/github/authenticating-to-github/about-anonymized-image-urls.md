...
Check the value of `Cache-Control`. In this example, there's no `Cache-Control`. In that case:
 ...
 If `Cache-Control` *is* set to `no-cache`, contact {% data variables.contact.contact_osmand_support%} or search the {% data variables.contact.community_support_forum %}.
