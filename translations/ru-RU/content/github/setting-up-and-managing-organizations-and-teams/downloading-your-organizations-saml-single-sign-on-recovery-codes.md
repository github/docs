...
intro: 'Organization administrators should download their organization''s SAML single sign-on recovery codes to ensure that they can access {{ site.data.variables.product.product_name }} even if the identity provider for the organization is unavailable.'
redirect_from:
...
6. Save your recovery codes by clicking **Download**, **Print**, or **Copy**. ![Buttons to download, print, or copy your recovery codes](/assets/images/help/saml/saml_recovery_code_options.png)
  ...
  **Note:** Your recovery codes will help get you back into {{ site.data.variables.product.product_name }} if your IdP is unavailable. If you generate new recovery codes the recovery codes displayed on the "Single sign-on recovery codes" page are automatically updated.

  {% endnote %}

7. Once you use a recovery code to regain access to {{ site.data.variables.product.product_name }}, it cannot be reused. Access to {{ site.data.variables.product.product_name }} will only be available for 24 hours before you'll be asked to sign in using single sign-on.
