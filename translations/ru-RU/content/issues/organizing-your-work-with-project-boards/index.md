---
title: 'Организация работы с помощью компонента "{% data variables.product.prodname_projects_v1 %}"'
shortTitle: '{% data variables.product.prodname_projects_v1_caps %}'
intro: 'Используйте {% data variables.product.prodname_projects_v1 %} для управления своей работой на {% data variables.product.prodname_dotcom %}.'
allowTitleToDifferFromFilename: true
versions:
  feature: projects-v1
topics:
  - Issues
  - Projects
children:
  - /managing-project-boards
  - /tracking-work-with-project-boards
redirect_from:
  - /adding-issues-and-pull-requests-to-a-project-board
  - /adding-notes-to-a-project-board
  - /tracking-progress-on-your-project-board
  - /filtering-cards-on-a-project-board
  - /archiving-cards-on-a-project-board
ms.openlocfilehash: 5827065f7fe316f4ec8ea41b56be61b1e01943dd
ms.sourcegitcommit: f638d569cd4f0dd6d0fb967818267992c0499110
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/25/2022
ms.locfileid: '148109040'
---
{% data reusables.projects.project_boards_old %}
