...
class Page {
  constructor(opts) {
    ...
    const { content, data, errors: frontmatterErrors } = frontmatter(this.raw, {
    ...
    const versionsParentProductIsNotAvailableIn = getApplicableVersions(
      this.versions,
      this.fullPath
    )
      ...
      .filter(
        (availableVersion) =>
          this.parentProduct &&
          !this.parentProduct.versions.includes(availableVersion)
      )

    if (
      versionsParentProductIsNotAvailableIn.length &&
  ...
  async _render(context) {
    ...
    this.includesPlatformSpecificContent =
      $('[class^="platform-"], .mac, .windows, .linux, .all').length > 0
