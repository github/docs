...
async function warmServer() {
  ...
  const [siteTree, pageList] = await Promise.all([
    dog.loadSiteTree(unversionedTree),
    dog.loadPages(unversionedTree)
  ])
  const [pageMap, redirects] = await Promise.all([
    dog.loadPageMap(pageList),
    dog.loadRedirects(pageList)
  ])
