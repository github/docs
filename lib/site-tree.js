...
function buildCategoriesTree(
  ...
  tocItems.forEach((item) => {
    ...
    const versionedCategoryHref = getVersionedPathWithoutLanguage(
      categoryHref,
      version
    )
    ...
    if (page.tocItems) {
      const hasMaptopics = page.tocItems.some(
        (item) => item.type === 'maptopic'
      )

      // if TOC contains maptopics, build a maptopics tree
      // otherwise build an articles tree
      if (hasMaptopics) {
        category.maptopics = buildMaptopicsTree(
          page.tocItems,
          categoryHref,
          pages,
          redirects,
        ...
        category.articles = buildArticlesTree(
          page.tocItems,
          categoryHref,
          pages,
          redirects,
...
function buildMaptopicsTree(
  ...
  tocItems
    ...
    .forEach((item) => {
      ...
      const versionedMaptopicHref = getVersionedPathWithoutLanguage(
        maptopicHref,
        version
      )
...
function buildArticlesTree(
  ...
  tocItems.forEach((item) => {
    ...
    const versionedArticleHref = getVersionedPathWithoutLanguage(
      articleHref,
      version
    )
