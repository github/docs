    ...
    for (const internalLink of internalLinks) {
      ...
      if (anchor && !link) {
        ...
        if (matchingHeadings.length === 0) {
          brokenLinks.anchors.push({
            'broken same-page anchor': `#${anchor}`,
            reason: 'heading not found on page'
          })
        }
      ...
      if (
        linkedPage.relativePath.includes('rest/reference/') &&
        linkedPage.relativePath !== 'rest/reference/index.md'
      ) {
        const linkedPageRelevantPermalink = linkedPage.permalinks.find(
          (permalink) => permalink.pageVersion === version
        )
      ...
      const linkedPageContent = linkedPage.relativePath.includes(
      ...
      if (anchorToCheck) {
        ...
        const matchingHeadings = getMatchingHeadings(
          linkedPageObject,
          anchorToCheck
        )
        ...
        if (matchingHeadings.length === 0) {
          if (anchor) {
            brokenLinks.anchors.push({
              ...
              'full link': `${getPathWithoutLanguage(link)}#${anchor}`,
              reason: 'heading not found on linked page',
              'linked page': linkedPage.fullPath
            })
          }
          continue
        }
