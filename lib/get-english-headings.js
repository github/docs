...
module.exports = function getEnglishHeadings(page, pages) {
  ...
  const cleanTranslatedHeadings = translatedHeadings.map((h) =>
    h.replace(headingRegex, '$1')
  )
  const cleanEnglishHeadings = englishHeadings.map((h) =>
    h.replace(headingRegex, '$1')
  )

  // return a map from translation:English
  return Object.assign(
    ...cleanTranslatedHeadings.map((k, i) => ({
