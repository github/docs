...
module.exports = async function siteTreeTitles(siteTree, siteData) {
  ...
  const titlesWithLiquid = Object.entries(flatTree).filter(
    ([path, title]) => path.endsWith('title') && title.includes('{')
  )

  await Promise.all(
    titlesWithLiquid.map(async ([path, title]) => {
      ...
      const currentCategory =
        siteTree[languageCode][version][products][product][categories][category]
