...
function getS3BucketPathFromVersion(version) {
  ...
  if (!versionObject) {
    console.error(
      `${version} is not a supported version, cannot get S3 bucket path`
    )
    return
  }
...
function getVersionFromS3BucketPath(bucketPath) {
  ...
  if (!versionObject) {
    console.error(
      `cannot find a supported version from S3 bucket path ${bucketPath}`
    )
    return
  }
