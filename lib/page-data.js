...
async function versionPages (obj, version, langCode, site) {
  ...
  // This only works for titles and short titles which have no other markup.
  ...
  const versionedChildPages = await Promise.all(obj.childPages
    ...
    .map(async (childPage) => await versionPages(Object.assign({}, childPage), version, langCode, site)))
