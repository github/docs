...
module.exports = async function findUnusedAssets(assetType) {
  assert(
    validArgs.includes(assetType),
    `arg must be one of: ${validArgs.join(', ')}`
  )
  ...
  const assetsReferencedInNonContentDirs = getAssetsReferencedInNonContentDirs(
    assetType
  )
  ...
  const allReferencedAssets = [
    ...new Set(
      assetsReferencedInContent.concat(assetsReferencedInNonContentDirs)
    )
  ]
...
function getAllImagesInRepo() {
  return walk(path.join(process.cwd(), imagesPath), { directories: false })
    .filter(
      (relPath) =>
        !relPath.endsWith('.md') && !relPath.match(/^(octicons|site)\//)
    )
