...
const inlineIfRegex = new RegExp(
  nonEmptyString.source + ifStatement.source,
  'gm'
)
...
module.exports = function removeLiquidStatements(
  content,
  versionToDeprecate,
  nextOldestVersion
) {
  ...
  const regexes = {
    ...
    lessThanNextOldestVersion: new RegExp(
      startTag.source +
        ` if .*?ver_lt ('|")${nextOldestVersion}('|") ?` +
        endTag.source,
      'm'
    ),
    equalsVersionToDeprecate: new RegExp(
      startTag.source +
        ` if .*?== ('|")${versionToDeprecate}('|") ?` +
        endTag.source,
      'm'
    )
  }
...
function removeLiquidOnly(content, allLiquidBlocks, regexes) {
  const blocksToUpdate = allLiquidBlocks.filter((block) => {
    ...
    if (block.match(regexes.greaterThanVersionToDeprecate))
      return (
        firstIf[0] === block.match(regexes.greaterThanVersionToDeprecate)[0]
      )
    if (block.match(regexes.andGreaterThanVersionToDeprecate1))
      return (
        firstIf[0] === block.match(regexes.andGreaterThanVersionToDeprecate1)[0]
      )
    if (block.match(regexes.andGreaterThanVersionToDeprecate2))
      return (
        firstIf[0] === block.match(regexes.andGreaterThanVersionToDeprecate2)[0]
      )
    ...
    if (block.match(regexes.andNotEqualsVersionToDeprecate))
      return (
        firstIf[0] === block.match(regexes.andNotEqualsVersionToDeprecate)[0]
      )
    return false
  })
  ...
  blocksToUpdate.forEach((block) => {
    ...
    if (newBlock.match(regexes.andGreaterThanVersionToDeprecate1)) {
      newBlock = newBlock.replace(
        regexes.andGreaterThanVersionToDeprecate1,
        matchAndStatement1
      )
    }
    ...
    if (newBlock.match(regexes.andGreaterThanVersionToDeprecate2)) {
      newBlock = newBlock.replace(
        regexes.andGreaterThanVersionToDeprecate2,
        matchAndStatement2
      )
    }
    ...
    if (newBlock.match(regexes.notEqualsVersionToDeprecate)) {
      newBlock = newBlock.replace(
        regexes.notEqualsVersionToDeprecate,
        matchNotEqualsStatement
      )
    }
    ...
    if (
      blockWithLeftContext.match(inlineIfRegex) &&
      block.match(inlineEndRegex)
    ) {
  ...
  blocksToRemove.forEach((block) => {
    ...
    if (elseBlock) {
      ...
      if (numberOfEndTags.length > 1) {
        ...
        newBlock = elseBlock[0]
          .replace(elseRegex, '')
          .replace(captureEndRegex, '')
      }
