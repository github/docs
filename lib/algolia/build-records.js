...
module.exports = async function buildRecords(
  indexName,
  indexablePages,
  pageVersion,
  languageCode
  ...
  const pages = indexablePages
    ...
    .filter((page) =>
      page.permalinks.some((permalink) => permalink.pageVersion === pageVersion)
    )
  ...
  const permalinks = pages
    .map((page) => {
      return page.permalinks.find((permalink) => {
        return (
          permalink.languageCode === languageCode &&
          permalink.pageVersion === pageVersion
        )
      })
    })
    .map((permalink) => {
