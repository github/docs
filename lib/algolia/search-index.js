...
class AlgoliaIndex {
  ...
  static get maxRecordLength() {
    return 8000
  }

  static get maxContentLength() {
    return 5000
  }
  ...
  validate() {
    ...
    assert(
      isArray(this.records) && this.records.length,
    ...
    assert(
      !dupes.length,
      `every objectID must be unique. dupes: ${dupes.join('; ')}`
    )

    this.records.forEach((record) => {
      assert(
        isString(record.objectID) && record.objectID.length,
        `objectID must be a string. received: ${
          record.objectID
      ...
      assert(
        ...
        `title must be a string. received: ${record.title}, ${JSON.stringify(
          record
        )}`
      )
      ...
      assert(
        isURL(record.url),
        `url must be a fully qualified URL. received: ${
          record.url
        }, ${JSON.stringify(record)}`
      )
  ...
  async syncWithRemote() {
    ...
    if (recordsToUpdate.length) {
      console.log(
        'uploading %d new or modified record(s)',
