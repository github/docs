...
if (!(fs.existsSync(awsCredsPath) || fs.existsSync(s3ConfigPath))) {
  console.error(
    'You need to set up awssume and s3cmd. Follow the steps at https://github.com/github/product-documentation/blob/master/doc-team-workflows/workflow-information-for-all-writers/setting-up-awssume-and-s3cmd.md'
  )
  process.exit(1)
}
...
module.exports = async function s3() {
  ...
  try {
    const bucket = buckets.Buckets.find(
      (bucket) => bucket.Name === 'github-images'
    )
    if (bucket) return s3
  } catch (e) {
...
async function listBuckets(s3) {
  try {
    ...
    if (e.code === 'ExpiredToken') {
      console.error(
        `${e.message} Generate new temporary creds with this command:\n\n$ awssume --profile s3-github-images`
      )
    } else {
...
function updateS3Creds() {
  ...
  if (
    !(
      s3configContents.includes(accessKey) &&
      s3configContents.includes(secretKey) &&
      s3configContents.includes(accessToken)
    )
  ) {
    console.log(
      'Updating your s3config file with your temporary AWS creds...\n'
    )
    newS3configContents = s3configContents

    newS3configContents = updateCred(
      'access_key',
      accessKey,
      newS3configContents
    )
