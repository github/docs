...
module.exports = class IndentedDataReference extends Liquid.Tag {
  async render(context) {
    ...
    assert(
      parseInt(numSpaces) || numSpaces === '0',
      '"spaces=NUMBER" must include a number'
    )
    ...
    const renderedReferenceWithIndent = value.replace(
      /^/gm,
      ' '.repeat(numSpaces)
    )

    return this.template.engine.parseAndRender(
