...
module.exports = {
  ...
  render: function * (ctx, emitter) {
    ...
    this.currentRelease = ctx.environments.currentVersion.split('@')[1]
    this.currentVersionShortName = ctx.environments.allVersions[ctx.environments.currentVersion].shortName
  ...
  handleOperators (resolvedBranchCond) {
    ...
    let resolvedBoolean
    if (operator === '!=') {
      // If this is the current version, compare the release numbers.
