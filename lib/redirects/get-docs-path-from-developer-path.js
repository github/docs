...
module.exports = function getDocsPathFromDeveloperPath(
  oldDeveloperPath,
  allRedirects,
  pages
) {
  ...
  if (newPath.endsWith(oauthAuthorizations)) {
    newPath = newPath.replace(
      oauthAuthorizations,
      '/enterprise-server/v3/oauth_authorizations'
    )
  }
  ...
  if (
    fragment &&
    newPath.includes('/rest/reference/enterprise-admin') &&
    !patterns.enterpriseServer.test(newPath)
  ) {
    newPath = getVersionedPathWithLanguage(
      newPath,
  ...
  if (
    ...
    !pages[newPath] &&
    pages['/en/rest/reference'] &&
    pages['/en/graphql/reference']
  ) {
