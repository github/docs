...
module.exports = async function precompileRedirects(pages) {
  ...
  ARCHIVED_ROUTES.forEach((archivedRoute) => {
    ...
    const versionFromPath = archivedRoute.match(
      patterns.getEnterpriseVersionNumber
    )[1]
  ...
  DEVELOPER_ROUTES.forEach((developerRoute) => {
    ...
    if (developerRoute.includes('/enterprise/')) {
      const developerRouteWithNewFormat = developerRoute.replace(
        /\/enterprise\/(\d.\d\d)\//,
        '/enterprise-server@$1/'
      )
    ...
    if (developerRoute.includes('/2.21/')) {
      ...
      allRedirects[
        developerRouteWithLanguageWithoutVersion
      ] = newPathOnLatestVersion
      // TODO after we update all the /v3 and /v4 links, we can yank the following
      const developerRouteWithoutVersionWithNewFormat = developerRouteWithoutVersion.replace(
        '/enterprise/',
        'enterprise-server'
      )
      const developerRouteWithoutVersionWithNewFormatWithLanguage = `/en${developerRouteWithoutVersionWithNewFormat}`
      allRedirects[
        developerRouteWithoutVersionWithNewFormat
      ] = newPathOnLatestVersion
      allRedirects[
        developerRouteWithoutVersionWithNewFormatWithLanguage
      ] = newPathOnLatestVersion
      // TODO ENDYANK
    }
    ...
    if (developerRoute.includes('/2.21/')) {
      ...
      const developerRouteWithNewFormat222 = developerRoute222.replace(
        '/enterprise/2.22/',
    ...
    if (developerRoute.includes('/enterprise/')) {
      const developerRouteWithUserPath = developerRoute.replace(
        /\/(v[34])/,
        '/user/$1'
      )
    ...
    if (!developerRoute.startsWith('/enterprise/')) {
      const developerRouteWithVersion = slash(
        path.join(nonEnterpriseDefaultVersion, developerRoute)
      )
