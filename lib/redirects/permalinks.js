...
module.exports = function generateRedirectsForPermalinks(
  permalinks,
  redirectFrontmatter
) {
  ...
  permalinks.forEach((permalink) => {
    ...
    redirectFrontmatterOldPaths.forEach((frontmatterOldPath) => {
      ...
      frontmatterOldPath = frontmatterOldPath.replace(
        patterns.trailingSlash,
        '$1'
      )

      // get the old path for the current permalink version
      const versionedFrontmatterOldPath = getVersionedPathWithLanguage(
        frontmatterOldPath,
        permalink.pageVersion,
        permalink.languageCode
      )
