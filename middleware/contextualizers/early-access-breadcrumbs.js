export default async function breadcrumbs(req, res, next) {
  ...
  const earlyAccessProduct = req.context.siteTree[req.language][
    req.context.currentVersion
  ].childPages.find((childPage) => childPage.page.relativePath === 'early-access/index.md')
  ...
  await createBreadcrumb(earlyAccessProduct.childPages, req.context)
...
async function createBreadcrumb(pageArray, context) {
  ...
  const hideHref =
    childPage.page.documentType === 'product' ||
    (childPage.page.documentType === 'category' &&
      childPage.page.relativePath.endsWith('/articles/index.md'))
