...
module.exports = async (req, res, next) => {
  ...
  if (!allReleaseNotes || !allReleaseNotes[versionString]) {
    return res.redirect(
      `https://enterprise.github.com/releases/${requestedVersion}.0/notes`
    )
  }
  ...
  const patches = keys
    ...
    .map((key) => ({
      version: `${requestedVersion}.${key}`,
      ...releaseNotes[key]
    }))

  const renderedPatches = await Promise.all(
    patches.map(async (patch) => {
      ...
      patch.notes = await Promise.all(
        patch.notes.map(async (note) => {
