...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    for (const fallbackRedirect of getFallbackRedirects(
      req,
      requestedVersion
    ) || []) {
      try {
...
function getProxyPath(reqPath, requestedVersion) {
  const proxyPath = versionSatisfiesRange(
    requestedVersion,
    `>=${firstVersionDeprecatedOnNewSite}`
  )
...
function getFallbackRedirects(req, requestedVersion) {
  ...
  const currentlySupportedPage = findPage(
    pathWithNewVersion,
    req.context.pages,
  ...
  return (
    Object.keys(currentlySupportedPage.redirects)
      ...
      .filter(
        (oldPath) =>
          oldPath.startsWith('/enterprise') &&
          patterns.enterpriseNoVersion.test(oldPath)
      )
      ...
      .map((oldPath) =>
        slash(
          path.join(
            ...
            oldPath.replace('/enterprise/', `/enterprise/${requestedVersion}/`)
          )
        )
      )
