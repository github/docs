...
describe('header', () => {
  ...
  describe('language links', () => {
    test('lead to the same page in a different language', async () => {
      ...
      expect(
        $(
          `#languages-selector a[href="/ja/${nonEnterpriseDefaultVersion}/github/administering-a-repository/enabling-required-status-checks"]`
        ).length
      ).toBe(1)
    })
    ...
    test('display the native name and the English name for each translated language', async () => {
      const $ = await getDOM('/en')
      expect($('#languages-selector a[href="/en"]').text().trim()).toBe(
        'English'
      )
      expect($('#languages-selector a[href="/cn"]').text().trim()).toBe(
        '简体中文 (Simplified Chinese)'
      )
      expect($('#languages-selector a[href="/ja"]').text().trim()).toBe(
        '日本語 (Japanese)'
      )
    })
  ...
  describe('notices', () => {
    ...
    test('displays "complete" notice for non-WIP non-English languages', async () => {
      ...
      expect($('.header-notifications a[href="/en"]').length).toBe(1)
      expect(
        $('.header-notifications a[href*="github.com/contact"]').length
      ).toBe(1)
    })
  ...
  describe('mobile-only product dropdown links', () => {
    test('include github and admin, and emphasize the current product', async () => {
      ...
      const github = $(
        `#homepages a.active[href="/en/${nonEnterpriseDefaultVersion}/github"]`
      )
      ...
      const ghe = $(
        `#homepages a[href="/en/enterprise-server@${latest}/admin"]`
      )
    ...
    test("point to homepages in the current page's language", async () => {
      ...
      expect(
        $(
          `#homepages a.active[href="/ja/${nonEnterpriseDefaultVersion}/github"]`
        ).length
      ).toBe(1)
      expect(
        $(`#homepages a[href="/ja/enterprise-server@${latest}/admin"]`).length
      ).toBe(1)
    })
    ...
    test('emphasizes the product that corresponds to the current page', async () => {
      ...
      expect(
        $(`#homepages a.active[href="/en/enterprise-server@${latest}/admin"]`)
          .length
      ).toBe(0)
      expect(
        $(`#homepages a[href="/en/${nonEnterpriseDefaultVersion}/github"]`)
          .length
      ).toBe(1)
      expect(
        $(
