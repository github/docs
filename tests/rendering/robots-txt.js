...
describe('robots.txt', () => {
  ...
  it('allows indexing of the homepage and English content', async () => {
    ...
    expect(
      robots.isAllowed(
        'https://help.github.com/en/articles/verifying-your-email-address'
      )
    ).toBe(true)
  })
  ...
  it('allows indexing of generally available localized content', async () => {
    Object.values(languages)
      ...
      .forEach((language) => {
        expect(
          robots.isAllowed(`https://help.github.com/${language.code}`)
        ).toBe(true)
        expect(
          robots.isAllowed(
            `https://help.github.com/${language.code}/articles/verifying-your-email-address`
          )
        ).toBe(true)
      })
  })
  ...
  it('allows indexing of non-WIP products', async () => {
    ...
    expect(
      robots.isAllowed('https://help.github.com/en/actions/overview')
    ).toBe(true)
    expect(
      robots.isAllowed('https://help.github.com/en/actions/overview/intro')
    ).toBe(true)
    expect(
      robots.isAllowed(
        `https://help.github.com/en/enterprise/${enterpriseServerReleases.latest}/user/actions`
      )
    ).toBe(true)
    expect(
      robots.isAllowed(
        `https://help.github.com/en/enterprise/${enterpriseServerReleases.oldestSupported}/user/actions`
      )
    ).toBe(true)
  })
