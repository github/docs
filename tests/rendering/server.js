...
describe('server', () => {
  ...
  test('sets Content Security Policy (CSP) headers', async () => {
    ...
    const csp = new (require('csp-parse'))(
      res.headers['content-security-policy']
    )
    expect(csp.get('default-src')).toBe("'none'")

    expect(csp.get('font-src').includes("'self'")).toBe(true)
    expect(csp.get('font-src').includes('github-images.s3.amazonaws.com')).toBe(
      true
    )
    ...
    expect(csp.get('img-src').includes('github-images.s3.amazonaws.com')).toBe(
      true
    )
  ...
  test('sets Fastly cache control headers', async () => {
    ...
    expect(res.headers['surrogate-control']).toBe(
      'max-age=86400, stale-if-error=600, stale-while-revalidate=600'
    )
  ...
  test('sets `lang` attribute on <html> attribute', async () => {
    ...
    expect((await getDOM('/en/articles/set-up-git'))('html').attr('lang')).toBe(
      'en'
    )
  ...
  test('renders a 404 page', async () => {
    ...
    expect($.text().includes("It looks like this page doesn't exist.")).toBe(
      true
    )
  ...
  test('renders a 500 page when errors are thrown', async () => {
    ...
    expect(
      $('code')
        .text()
        .includes(path.join('node_modules', 'express', 'lib', 'router'))
    ).toBe(true)
    expect($.text().includes('Still need help?')).toBe(true)
  ...
  test('injects site variables into rendered intros', async () => {
    ...
    const $ = await getDOM(
      '/en/github/getting-started-with-github/following-people'
    )
  ...
  test('renders Markdown links that have Liquid hrefs', async () => {
    ...
    expect($('a[href="https://github.com/pricing"]').first().text()).toBe(
      'Pricing'
    )
  })
  ...
  test('renders liquid within liquid within liquid in body text', async () => {
    const $ = await getDOM(
      '/en/github/administering-a-repository/enabling-required-status-checks'
    )
  ...
  test('renders liquid within liquid within liquid in intros', async () => {
    const $ = await getDOM(
      '/en/github/administering-a-repository/about-merge-methods-on-github'
    )
    expect(
      $('div.lead-mktg')
        .first()
        .text()
        .includes('merge their pull requests on GitHub')
    ).toBe(true)
  })
  ...
  test('preserves liquid statements with liquid raw tags in page output', async () => {
    const $ = await getDOM(
      '/en/github/working-with-github-pages/troubleshooting-jekyll-build-errors-for-github-pages-sites'
    )
  ...
  test('displays links to categories on product TOCs', async () => {
    ...
    expect(
      $(
        `article a[href="/en/${nonEnterpriseDefaultVersion}/github/getting-started-with-github"]`
      )
    ).toHaveLength(1)
  })
  ...
  describe('autogenerated mini TOCs', () => {
    ...
    test('renders mini TOC in articles with more than one heading', async () => {
      const $ = await getDOM(
        '/en/github/getting-started-with-github/githubs-products'
      )
    ...
    test('does not render mini TOC in articles with no headings', async () => {
      const $ = await getDOM(
        '/en/github/authenticating-to-github/reviewing-your-deploy-keys'
      )
      expect($('h3#in-this-article').length).toBe(0)
    })
  ...
  describe('image asset paths', () => {
    ...
    test('github articles on dotcom have images that point to local assets dir', async () => {
      ...
      expect($('img').first().attr('src').startsWith(localImageBasePath)).toBe(
        true
      )
    })
    ...
    test('github articles on GHE have images that point to S3', async () => {
      ...
      expect(
        $('img').first().attr('src').startsWith(enterpriseImageBasePath)
      ).toBe(true)
    })
    ...
    test('github articles on GHAE have images that point to S3', async () => {
      ...
      expect(
        $('img')
          .first()
          .attr('src')
          .startsWith(`${s3BasePath}/github-ae/assets`)
      ).toBe(true)
    })

    test('admin articles on GHAE have images that point to S3', async () => {
      const $ = await getDOM(
        '/en/github-ae@latest/admin/user-management/managing-dormant-users'
      )
      expect(
        $('img')
          .first()
  ...
  describe('English local links', () => {
    ...
    test('dotcom articles on dotcom have links that include "en"', async () => {
      ...
      expect(
        $(
          `a[href="/en/${nonEnterpriseDefaultVersion}/articles/managing-files-on-github"]`
        ).length
      ).toBe(1)
    })
    ...
    test('dotcom articles on dotcom have Enterprise Admin links with latest GHE version', async () => {
      const $ = await getDOM(
        '/en/articles/setting-up-a-trial-of-github-enterprise-server'
      )
      expect(
        $(
    ...
    test('dotcom articles on GHE have Enterprise user links', async () => {
      ...
      expect(
        $(`a[href="${latestEnterprisePath}/articles/managing-files-on-github"]`)
          .length
      ).toBe(1)
    })
    ...
    test('dotcom categories on GHE have Enterprise user links', async () => {
      const $ = await getDOM(
        `${latestEnterprisePath}/github/managing-large-files`
      )
      expect(
        $(
          `article a[href="${latestEnterprisePath}/github/managing-large-files/conditions-for-large-files"]`
        ).length
      ).toBe(1)
    })
    ...
    test('dotcom-only links on GHE are dotcom-only', async () => {
      ...
      expect(
        $(
          `article a[href="/en/${nonEnterpriseDefaultVersion}/articles/github-privacy-statement"]`
        ).length
      ).toBe(1)
    })
    ...
    test('desktop links on GHE are dotcom-only', async () => {
      ...
      expect(
        $(
          `article a[href="/en/${nonEnterpriseDefaultVersion}/desktop/installing-and-configuring-github-desktop"]`
        ).length
      ).toBe(1)
    })
    ...
    test('admin articles that link to Enterprise user articles have Enterprise user links', async () => {
      const $ = await getDOM(
        `${latestEnterprisePath}/admin/user-management/configuring-email-for-notifications`
      )
      expect(
        $(
          'article a[href*="about-email-notifications-for-pushes-to-your-repository"]'
        ).length
      ).toBe(1)
    })
  ...
  describe('article versions', () => {
    test('includes links to all versions of each article', async () => {
      ...
      expect(
        $(`.article-versions a.active[href="/en/${articlePath}"]`).length
      ).toBe(0)

      // 2.13 predates this feature, so it should be excluded:
      expect(
        $(`.article-versions a[href="/en/enterprise/2.13/user/${articlePath}"]`)
          .length
      ).toBe(0)
    })
  ...
  describe('redirects', () => {
    ...
    test('redirects old articles to their slugified URL', async () => {
      ...
      expect(res.text).toBe(
        `Moved Permanently. Redirecting to /en/${nonEnterpriseDefaultVersion}/github/authenticating-to-github/about-githubs-ip-addresses`
      )
    })
    ...
    test.skip('redirects versioned category page', async () => {
      const res = await get(
        '/en/github/receiving-notifications-about-activity-on-github'
      )
      expect(res.statusCode).toBe(301)
      expect(res.headers.location).toBe(
  ...
  describe('categories and map topics', () => {
    test('adds links to categories on the dotcom homepage', async () => {
      ...
      expect(
        $(
          `article a[href="/en/${nonEnterpriseDefaultVersion}/github/managing-large-files"]`
        ).length
      ).toBe(1)
    ...
    test('adds links to map topics on a category homepage', async () => {
      const $ = await getDOM(
        '/en/github/setting-up-and-managing-your-github-user-account'
      )
      expect(
        $(
          `article a[href="/en/${nonEnterpriseDefaultVersion}/github/setting-up-and-managing-your-github-user-account/managing-user-account-settings"]`
        ).length
      ).toBe(1)
      expect(
        $('article a[href="#managing-user-account-settings"]').length
      ).toBe(0)
    })
    ...
    test('map topic renders with h4 links to articles', async () => {
      ...
      expect(
        $(
          `a[href="/en/${nonEnterpriseDefaultVersion}/github/setting-up-and-managing-your-github-user-account/changing-your-github-username"] h4`
        ).length
      ).toBe(1)
    })
    ...
    test('map topic intros are parsed', async () => {
      ...
      expect($intro.html()).toContain(
        'Use the <strong>Available for hire</strong>'
      )
    })
  })
})
...
describe('URLs by language', () => {
  ...
  test.skip('heading IDs and links on translated pages are in English', async () => {
    const $ = await getDOM(
      '/ja/github/getting-started-with-github/verifying-your-email-address'
    )
...
describe('GitHub Enterprise URLs', () => {
  test('renders the GHE user docs homepage', async () => {
    ...
    expect(
      $(
        `article a[href="/en/enterprise-server@${enterpriseServerReleases.latest}/github/getting-started-with-github"]`
      ).length
    ).toBe(1)
  })
  ...
  test('renders an Enterprise Admin category with correct links', async () => {
    ...
    expect(
      $(`article a[href^="${installationCategoryHome}/"]`).length
    ).toBeGreaterThan(1)
  })
  ...
  test('renders Enterprise homepage in Japanese', async () => {
    const res = await get(
      `/ja/enterprise-server@${enterpriseServerReleases.latest}`
    )
    expect(res.statusCode).toBe(200)
  })
  ...
  test('renders Enterprise User homepage in Japanese', async () => {
    const res = await get(
      `/ja/enterprise-server@${enterpriseServerReleases.latest}/github`
    )
    expect(res.statusCode).toBe(200)
  })
  ...
  test('renders Enterprise Admin homepage in Japanese', async () => {
    const res = await get(
      `/ja/enterprise-server@${enterpriseServerReleases.latest}/admin`
    )
    expect(res.statusCode).toBe(200)
  })
  ...
  test('renders Enterprise homepage in Chinese', async () => {
    const res = await get(
      `/cn/enterprise-server@${enterpriseServerReleases.latest}`
    )
    expect(res.statusCode).toBe(200)
  })
  ...
  test('renders Enterprise User homepage in Chinese', async () => {
    const res = await get(
      `/cn/enterprise-server@${enterpriseServerReleases.latest}/github`
    )
    expect(res.statusCode).toBe(200)
  })
  ...
  test('renders Enterprise Admin homepage in Chinese', async () => {
    const res = await get(
      `/cn/enterprise-server@${enterpriseServerReleases.latest}/admin`
    )
    expect(res.statusCode).toBe(200)
  })
})
...
describe('GitHub Desktop URLs', () => {
  test('renders the GitHub Desktop homepage with correct links', async () => {
    ...
    expect(
      $(`article a[href^="/en/${nonEnterpriseDefaultVersion}/desktop/"]`).length
    ).toBeGreaterThan(1)
  })

  test('renders a Desktop category with expected links', async () => {
    const $ = await getDOM(
      '/en/desktop/installing-and-configuring-github-desktop'
    )
    expect(
      $(
        `article a[href^="/en/${nonEnterpriseDefaultVersion}/desktop/installing-and-configuring-github-desktop/"]`
  ...
  test('renders a Desktop article within a map topic', async () => {
    const res = await get(
      `/en/${nonEnterpriseDefaultVersion}/desktop/installing-and-configuring-github-desktop/installing-github-desktop`
    )
    expect(res.statusCode).toBe(200)
  })
...
describe('extended Markdown', () => {
  ...
  test('renders styled notes within liquid', async () => {
    const $ = await getDOM(
      '/en/articles/removing-a-member-from-your-organization'
    )
  ...
  test('renders platform-specific content', async () => {
    const $ = await getDOM(
      '/en/github/using-git/associating-text-editors-with-git'
    )
    expect(
      $('.extended-markdown.mac h3#using-textmate-as-your-editor').length
    ).toBe(1)
    expect(
      $('.extended-markdown.windows h3#using-notepad-as-your-editor').length
    ).toBe(1)
    expect(
      $('.extended-markdown.linux h3#using-textmate-as-your-editor').length
    ).toBe(0)
    expect(
      $('.extended-markdown.linux h3#using-notepad-as-your-editor').length
    ).toBe(0)
  })
  ...
  test('renders expected mini TOC headings in platform-specific content', async () => {
    ...
    expect($('h3#in-this-article').length).toBe(1)
    expect(
      $('h3#in-this-article + ul li.extended-markdown.mac').length
    ).toBeGreaterThan(1)
    expect(
      $('h3#in-this-article + ul li.extended-markdown.windows').length
    ).toBeGreaterThan(1)
    expect(
      $('h3#in-this-article + ul li.extended-markdown.linux').length
    ).toBeGreaterThan(1)
  })
})
...
describe('search', () => {
  function findDupesInArray(arr) {
    return lodash.filter(arr, (val, i, iteratee) =>
      lodash.includes(iteratee, val, i + 1)
    )
  }
...
describe('?json query param for context debugging', () => {
  ...
  it('returns a helpful message with top-level keys if query param has no value', async () => {
    ...
    expect(
      context.message.includes('context object is too big to display')
    ).toBe(true)
...
describe('static routes', () => {
  it('serves content from the /assets directory', async () => {
    expect((await get('/assets/images/site/be-social.gif')).statusCode).toBe(
      200
    )
  })
  ...
  it('serves schema files from the /data/graphql directory at /public', async () => {
    ...
    expect(
      (
        await get(
          `/public/ghes-${enterpriseServerReleases.latest}/schema.docs-enterprise.graphql`
        )
      ).statusCode
    ).toBe(200)
    expect(
    ...
    expect(
      (await get('/public/ghae/schema.docs-ghae.graphql')).statusCode
    ).toBe(200)
  })

  it('does not serve repo contents that live outside the /assets directory', async () => {
    expect(
      (await get('/package.json', { followRedirects: true })).statusCode
    ).toBe(404)
    expect(
      (await get('/README.md', { followRedirects: true })).statusCode
    ).toBe(404)
    expect(
      (await get('/server.js', { followRedirects: true })).statusCode
    ).toBe(404)
  })
})
...
describe('index pages', () => {
  ...
  test('excludes dotcom-only from GHE TOC', async () => {
    const $ = await getDOM(
      `/en/enterprise/${enterpriseServerReleases.latest}/user/github/getting-started-with-github`
    )
  ...
  test('includes correctly versioned links in GHE', async () => {
    ...
    expect(
      $(`article a[href^="${installationLatest}/"]`).length
    ).toBeGreaterThan(0)
  })
})
