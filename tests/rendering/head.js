...
describe('<head>', () => {
  ...
  test('includes page intro in `description` meta tag', async () => {
    ...
    expect(
      $description.attr('content').startsWith('Using the SSH protocol')
    ).toBe(true)
  })
  ...
  test('renders `description` meta tag in plaintext (no HTML)', async () => {
    ...
    expect(
      $description
        .attr('content')
        .startsWith('You can merge pull requests by retaining')
    ).toBe(true)
    // HTML intro
    expect(
      $('div.lead-mktg')
        .html()
        .startsWith(
          '<p>You can <a href="/articles/merging-a-pull-request">merge pull requests</a>'
        )
    )
  })
})
