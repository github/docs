...
describe('featuredLinks', () => {
  ...
  describe('rendering', () => {
    ...
    test('landing page intro links have expected properties', async () => {
      ...
      expect($featuredLinks.eq(0).attr('href')).toBe(
        `/en/${nonEnterpriseDefaultVersion}/github/getting-started-with-github/set-up-git`
      )
      expect(
        $featuredLinks.eq(0).children('h4').text().startsWith('Set up Git')
      ).toBe(true)
      expect(
        $featuredLinks
          .eq(0)
          .children('p')
          .text()
          .startsWith('At the heart of GitHub')
      ).toBe(true)

      expect($featuredLinks.eq(8).attr('href')).toBe(
        `/en/${nonEnterpriseDefaultVersion}/github/working-with-github-pages`
      )
      expect(
        $featuredLinks.eq(8).children('h4').text().startsWith('GitHub Pages')
      ).toBe(true)
      expect(
        $featuredLinks
          .eq(8)
          .children('p')
          .text()
          .startsWith('You can create a website')
      ).toBe(true)
    })
    ...
    test('localized intro links link to localized pages', async () => {
      ...
      expect(
        japaneseCharacters.presentIn($featuredLinks.eq(0).children('h4').text())
      ).toBe(true)
      expect(
        japaneseCharacters.presentIn($featuredLinks.eq(0).children('p').text())
      ).toBe(true)
    })

    test('Enterprise user intro links have expected values', async () => {
      const $ = await getDOM(
        `/en/enterprise/${enterpriseServerReleases.latest}/user/insights`
      )
      const $featuredLinks = $('.featured-links a')
      ...
      expect(
        $featuredLinks
          ...
          .children('h4')
          .text()
          .startsWith('About GitHub Insights')
      ).toBe(true)
      expect(
        $featuredLinks
          .eq(0)
          .children('p')
          .text()
          .startsWith('GitHub Insights provides metrics')
      ).toBe(true)
    })

    test('featured links respect versioning', async () => {
      const $ = await getDOM(
        `/en/enterprise/${enterpriseServerReleases.latest}/user/packages`
      )
      const $featuredLinks = $('.featured-links a')
      expect($featuredLinks.length).toBeGreaterThan(2)
      // does not include dotcom-only links
      expect(
        $featuredLinks.text().includes('About GitHub Container Registry')
      ).toBe(false)
      expect(
        $featuredLinks
          .text()
          .includes('Getting started with GitHub Container Registry')
      ).toBe(false)
    })
  })
  ...
  describe('context.page object', () => {
    test('returns modified array of links', async () => {
      const gettingStartedLinks = await getJSON(
        '/en?json=featuredLinks.gettingStarted'
      )
      ...
      expect(
        gettingStartedLinks[0].intro.startsWith('At the heart of GitHub')
      ).toBe(true)
    })

    test('returns raw array of links on the page object', async () => {
      const rawGettingStartedLinks = await getJSON(
        '/en?json=page.featuredLinks.gettingStarted'
      )
      expect(rawGettingStartedLinks[0]).toEqual(
