...
describe('webhook payloads', () => {
  ...
  test('have payloads that are JSON strings, not objects', () => {
    ...
    const firstKey = Object.keys(
      webhookPayloads[nonEnterpriseDefaultPayloadVersion]
    )[0]
    const firstValue =
      webhookPayloads[nonEnterpriseDefaultPayloadVersion][firstKey]
  ...
  test('on non-dotcom versions, dotcom-only payloads fall back to dotcom', async () => {
    ...
    const ghesPayloadString = getPayloadString(
      ghesPayloadsWithFallbacks[dotcomOnlyPayload]
    )
    const ghaePayloadString = getPayloadString(
      ghaePayloadsWithFallbacks[dotcomOnlyPayload]
    )
